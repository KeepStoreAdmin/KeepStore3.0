...
Partial Public Class _Default
    ...
    Private Sub EnsureHomePoolsFallback(ByRef offerPool As DataTable,
        ...
        Dim allEmpty As Boolean =
            IsTableEmpty(offerPool) AndAlso
            IsTableEmpty(featuredPool) AndAlso
            IsTableEmpty(mostViewedPool) AndAlso
            IsTableEmpty(newArrivalsPool) AndAlso
            IsTableEmpty(bestSellerPool) AndAlso
            IsTableEmpty(topSellingPool)
        ...
        offerPool = genericFallback.Copy()
        featuredPool = genericFallback.Copy()
        mostViewedPool = ShuffleTable(genericFallback.Copy())
        newArrivalsPool = genericFallback.Copy()
        bestSellerPool = genericFallback.Copy()
        topSellingPool = ShuffleTable(genericFallback.Copy())
    End Sub
