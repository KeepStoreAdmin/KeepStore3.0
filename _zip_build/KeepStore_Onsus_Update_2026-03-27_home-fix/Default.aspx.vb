...
Partial Public Class _Default
    ...
    Private Function GetFeaturedPool(ByVal limit As Integer) As DataTable
        Return QueryProducts("COALESCE(v.Vetrina,0)=1 AND " & StockWhereClause() & ">=1 AND (" &
    ...
    Private Function GetMostViewedPool(ByVal limit As Integer) As DataTable
        Return QueryProducts(StockWhereClause() & ">=1 AND COALESCE(v.Visite,0)>0", "COALESCE(v.Visite,0) DESC, COALESCE(v.DataCreazione,CURDATE()) DESC, v.id DESC", limit)
    End Function

    Private Function GetNewArrivalsPool(ByVal limit As Integer) As DataTable
        Return QueryProducts(StockWhereClause() & ">=1 AND COALESCE(v.DataCreazione,'1900-01-01') >= DATE_SUB(CURDATE(), INTERVAL 365 DAY)", "COALESCE(v.DataCreazione,CURDATE()) DESC, COALESCE(v.Visite,0) DESC, v.id DESC", limit)
    End Function
    ...
    Private Function TryLoadProducts(ByVal whereClause As String, ByVal orderClause As String, ByVal limit As Integer) As DataTable
        ...
        sql.Append("AND COALESCE(v.Attivo,1)=1 AND COALESCE(v.Abilitato,1)=1 ")
