...
Assert-Source ($readModel -match 'CartStorefrontOwnerContext\.Resolve\(context\)') 'CART_READ_MODEL_TENANT_SCOPED'
Assert-Source ($snapshot -match 'CartAuthoritativeReadModel\.GetCurrent') 'CART_SNAPSHOT_TENANT_SCOPED'
Assert-Source ($revalidation -match 'CartStorefrontOwnerContext\.ResolveForMutation') 'CART_REVALIDATION_TENANT_SCOPED'
Assert-Source ($cart -notmatch 'sdsArticoli\.SelectCommand') 'CART_PAGE_TENANT_SCOPED'
Assert-Source ($miniCart -match 'CartAuthoritativeReadModel\.GetCurrent\(HttpContext\.Current\)') 'MINICART_TENANT_SCOPED'
...
Assert-Source ($readModel -notmatch 'context\.Session\.SessionID') 'CART_SNAPSHOT_RAW_SESSION_ABSENT'
