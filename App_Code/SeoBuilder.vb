...
' ============================================================
' SEO Builder (KeepStore 3.0) - WebForms safe
' FIX: rimosso ISeoMaster da qui per evitare conflitti BC30175
' - Canonical, Meta, OpenGraph, JSON-LD
' - VB.NET valido (Framework 4.x / VB 2012)
' ============================================================
...
Public NotInheritable Class SeoBuilder
    ...
    ' Chiamata compatibile con il tuo Default.aspx.vb:
    ...
    Public Shared Sub ApplyHomeSeo(ByVal page As Page)
        ...
        Dim jsonLd As String = BuildHomeJsonLdMinimal(page, title, description, canonical)
        InjectJsonLdIntoMaster(page, jsonLd)
    End Sub
    ...
    Public Shared Function BuildHomeJsonLdMinimal(ByVal page As Page, ByVal title As String, ByVal description As String, ByVal canonicalUrl As String) As String
        ...
        sb.Append("""name"":" & JsonString(siteName) & ",")
        ...
        sb.Append("""@id"":""" & JsonString(canonicalUrl & "#webpage") & """,")
        sb.Append("""url"":""" & JsonString(canonicalUrl) & """,")
        sb.Append("""name"":" & JsonString(title) & ",")
        sb.Append("""description"":" & JsonString(description) & ",")
        ...
        sb.Append("""@id"":""" & JsonString(canonicalUrl & "#breadcrumb") & """,")
    ...
    ' ============================================================
    ' MASTER INJECTION (NO ISeoMaster dependency)
    ' ============================================================

    Private Shared Sub InjectJsonLdIntoMaster(ByVal page As Page, ByVal jsonLd As String)
        If page Is Nothing Then Return
        If String.IsNullOrWhiteSpace(jsonLd) Then Return
        If page.Master Is Nothing Then Return

        ' 1) Provo ad impostare una property SeoJsonLd se esiste (senza interfacce -> evita conflitti)
        Try
            CallByName(page.Master, "SeoJsonLd", CallType.Set, jsonLd)
            Return
        Catch
            ' Ignoro: la master potrebbe non avere la property
        End Try

        ' 2) Fallback: cerco Literal litSeoJsonLd (se presente in <head>)
