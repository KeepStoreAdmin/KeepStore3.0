(function () {
  ...
  function initMobileCatalogMenu() {
    ...
    function buildMobileCategoryHtml(column, sectorId) {
      var headingLink = column.querySelector('.ks-desktop-category-heading');
      var categoryLabel = textOf(headingLink);
      var categoryUrl = headingLink ? (headingLink.getAttribute('href') || '#') : '#';
      var tipologyLinks = Array.prototype.slice.call(column.querySelectorAll('.ks-desktop-tipology-link'));
    ...
    function buildMobileSectorHtml(sectorItem) {
      var sectorLink = findDirectChild(sectorItem, 'A', 'item-link');
      ...
      var categoryColumns = panel ? Array.prototype.slice.call(panel.querySelectorAll('.ks-desktop-category-column')) : [];
      ...
      var html = [
        ...
        '<li><a class="sub-nav-link active" href="', escapeHtml(sectorUrl), '">Vedi tutto il reparto</a></li>'
      ];
