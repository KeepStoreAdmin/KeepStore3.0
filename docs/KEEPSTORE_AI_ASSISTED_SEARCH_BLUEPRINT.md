...
Checkpoint corrente 2026-09-10: `frontend-rebuild` / `origin/frontend-rebuild` a `b5206fa247580d76843387dd1036c611df384302`; `main` / `origin/main` invariati a `976e99f17cabc8a5c6a8715463444edfeaadcd91`. `WORKFLOW-GOVERNANCE-1A` e `CART-BATCH-ATOMICITY-1A` sono chiusi e integrati. Le directory non tracciate `Public/assets/images/articoli/`, `Public/assets/images/marche/` e `Public/assets/images/settori/` restano escluse dai commit. Anche eventuali loghi locali non tracciati sotto `Public/assets/images/vettori/` vanno preservati e possono essere aggiunti solo se nominativamente autorizzati da uno specifico manifest.
...
Stato promo/cart corrente: `HOTFIX-STOREFRONT-PROMO-DEMO-BADGE-1A`, `PROMO-COMMERCIAL-CORRECTNESS-1A`, `STOREFRONT-PROMO-ROUTES-HOTFIX-1A`, `PROMO-OWNER-ENFORCEMENT-1A REV2.7A` e `CART-BATCH-ATOMICITY-1A` sono CHIUSI / A. Il carrello batch determina prezzi, promozioni, owner e listino server-side e conclude atomicamente tutte le righe oppure nessuna; questa chiusura non dichiara complete le route promo, lo storefront, search, SEO o AI.
...
Roadmap vigente: `CART-BATCH-ATOMICITY-1A` non e piu un debito aperto. Il prossimo cart task raccomandato e `CART-DEADLOCK-RETRY-1A`, seguito da `CART-IDEMPOTENCY-PERSISTENCE-AUDIT-1A`, mobile cart UX, touch target rimozione `44x44` e audit monetari `DOUBLE`; HOME TTFB e audit dipendenze MySQL restano separati. Parity/retirement delle route promo richiede ancora prova dedicata. Coupon/Groupon resta differito a `COUPON-CART-LIFECYCLE-1A` e non va attivato senza richiesta esplicita Germano. SEO tecnico, Google Product structured data e AI/Gemini/LLMS restano finali; `LLMS-TXT-ASHX-JSONLD-AUDIT-1` e ancora solo futuro read-only. Feed, JSON-LD, LLMS, Gemini e structured data non sono stati modificati. HOME, catalogo, PDP e promo non sono dichiarati completi.

Priorita normativa trasversale: `B2C-WITHDRAWAL-COMPLIANCE-AUDIT-1A` precede ora i normali debiti tecnici ed e il prossimo audit consigliato; `B2C-WITHDRAWAL-FUNCTION-1A` puo essere un release blocker. Non appartiene alla coda AI/SEO e non riapre cart o promo chiusi. Coupon/Groupon resta differito.
...
Checkpoint cart rilevante ma non estensivo della roadmap AI: `CART-BATCH-ATOMICITY-1A` e chiuso; prezzi, promozioni, owner e listino del batch sono server-side. Il prossimo debito cart e `CART-DEADLOCK-RETRY-1A`, non un task AI. La chiusura non rende KeepStore `AI ready`, `SEO completo` o `Google pronto` e non modifica feed, JSON-LD, LLMS, Gemini o structured data.

La priorita normativa `B2C-WITHDRAWAL-COMPLIANCE-AUDIT-1A` viene prima di questa roadmap AI. Nessun task AI o SEO puo essere usato come sostituto della funzione digitale di recesso o trattarne i dati in retrieval pubblico.
