...
`MULTIPROVIDER-TENANT-EMAIL-ALL-CALLERS-MIGRATION-1A-REV1` completa il checkpoint applicativo della PR #268: ordine/BCC, registrazione/profilo, reset password tokenizzato e contatti moderno/legacy usano un solo facade tenant-scoped con `Purpose=TRANSACTIONAL`; le code documento restano un confine esterno dichiarato. Nessun chiamante legge SMTP legacy o conserva credenziali in Session. Il runner amministrativo locale riusa il runtime canonico per TLS/AUTH/QUIT senza messaggio, endpoint pubblico o aggiornamento DB e restituisce solo esiti sanitizzati. In `taikun` i due profili restano disabilitati, `NOT_VERIFIED`, privi di credenziali reali e associati a placeholder non operativi, quindi runner e invii falliscono chiusi prima di SMTP. Provisioning, rotazione, verifica reale e attivazione restano un passaggio operativo separato `NON AVVIATO`; OAuth2 Google/Microsoft resta `NON OPERATIVO`.
