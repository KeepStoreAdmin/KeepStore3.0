...
Stato promo corrente: `HOTFIX-STOREFRONT-PROMO-DEMO-BADGE-1A`, `PROMO-COMMERCIAL-CORRECTNESS-1A` e `STOREFRONT-PROMO-ROUTES-HOTFIX-1A` sono CHIUSI / A con smoke Germano A. L'audit route read-only e COMPLETATO / A e classifica `promozioni.aspx` come legacy C, parzialmente sovrapposta ma non equivalente alla moderna `articoli.aspx?inpromo=1`. Il finding `Demo` e superato; promo principale solo se valida, attiva, positiva, applicabile a quantita 1 e inferiore al prezzo base. Tier, `QntMinima` e `Multipli` restano comunicati separatamente e il carrello e la fonte di verita.
...
Roadmap vigente: `STOREFRONT-PROMO-MODERN-PARITY-1A`; `STOREFRONT-PROMO-ROUTE-RETIREMENT-1A` solo dopo parita funzionale dimostrata; `STOREFRONT-OFFERS-PROMO-UX-1A`; `HOME-ASYNC-CART-1A`; `PDP-BRAND-LOGO-1A`; quindi performance/bulk promo, SEO tecnico, Google Product structured data e infine AI/Gemini/LLMS. `LLMS-TXT-ASHX-JSONLD-AUDIT-1` resta read-only e finale: nessun `llms.txt`, `llms.ashx`, API AI o JSON-LD dinamico e implementato. `articolix.aspx` 500, i due `EAN:` in `ProductDetailView.ascx`, add-to-cart/overflow/404/output encoding legacy, materializzazione e N+1 restano backlog separati. HOME, catalogo, PDP e promo non sono dichiarati completi.
