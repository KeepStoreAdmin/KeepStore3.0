...
Stato stabile trasversale dopo `MERGE-STOREFRONT-CATALOG-MOBILE-CART-BLOCK-2`: `frontend-rebuild` / `origin/frontend-rebuild` a `b31f8177e09cffcd5e9193e1e26eb3bbec4f8e6b`; `main` / `origin/main` invariati a `976e99f17cabc8a5c6a8715463444edfeaadcd91`. Base cumulativa precedente `300b97a7a8873d3cd0cb750be2e949523a8ebaa0`; consolidamento fast-forward only di 7 commit, ahead/behind `0 / 7`. Le directory non tracciate `Public/assets/images/articoli/` e `Public/assets/images/settori/` restano escluse dai commit. Gli SHA catalog parity precedenti restano riferimenti storici corretti. Regola operativa: ogni branch runtime Codex va pushato su origin prima del report finale/review ChatGPT, salvo impedimento tecnico documentato; ChatGPT legge i tre manuali e verifica direttamente commit/diff GitHub prima dell'approvazione, mentre Codex non mergea senza via libera ChatGPT/Germano.
...
- Prossimo runtime ufficiale `STOREFRONT-AVAILABILITY-PRESENTATION-1A`: source of truth `AvailabilityDisplayHelper`, rispetto `Session("DispoTipo")`, presentazione coerente HOME/catalogo/PDP/carrello, niente N+1 e niente business logic browser. Include titoli HOME circa tre righe; il titolo PDP resta completo. Side cart, parity P1/P2 e blocchi ecommerce restano separati; SEO/AI/Gemini rimane finale.
