...
Regola stato: chiudere un micro-task su `articoli.aspx` non significa dichiarare completa la pagina catalogo. Sono chiusi micro-task su card, selezione multipla, quantita/delta/visual mobile in carrello, CTA `Acquista`, stato/paging deterministico, toolbar/quattro viste e product-image 404; search/suggest, zero-results, sidebar/facet e filtri applicati mantengono i perimetri documentati. `CATALOG-ONSUS-PARITY-AUDIT-1` e completato e i tre P0 sono chiusi, ma full ONSUS parity e gap P1/P2 restano aperti.

Nota anti-false-closure: `articoli.aspx` non e una pagina dichiarata completa. I micro-task chiusi non equivalgono alla parita ONSUS completa. Restano sidebar/facet residui, tassonomie/load-more, active filters legacy, Price/Deals/Condition, performance, componenti commerciali e responsive complessivo.

Stato stabile trasversale dopo `CATALOG-ONSUS-PARITY-1C-IMAGE-PATH-404`: `frontend-rebuild` / `origin/frontend-rebuild` a `9a6e5b1babc836adb33cd43f930a112cd6e77103`; `main` / `origin/main` invariati a `976e99f17cabc8a5c6a8715463444edfeaadcd91`. Restano storici corretti `1acbba90...` (`1A`), `9903687...` (`1B`) e `817966ed...` (`1C` principale). Regola operativa: ogni branch runtime Codex va pushato su origin prima del report finale/review ChatGPT, salvo impedimento tecnico documentato; ChatGPT legge i tre manuali e verifica direttamente commit/diff GitHub prima dell'approvazione, mentre Codex non mergea senza via libera ChatGPT/Germano.
...
- `GLOBAL-TYPOGRAPHY-ONSUS-1C` e NON NECESSARIO / NON CREATO e il blocco typography storefront e CHIUSO. Questa chiusura non dichiara complete HOME, catalogo, PDP, header o footer. Audit catalog parity e runtime `1A/1B/1C` sono chiusi; `articoli.aspx` resta esplicitamente non completa. Il prossimo runtime e `PDP-TITLE-SHIPPING-INFO-1A`, side cart resta successivo e Gemini/LLMS resta Priorita 8/finale. Nessuna query, logica search/AI, ranking o runtime AI e stata modificata dal blocco typography o dalla sola riconciliazione docs.
- Finding PDP da coda immediata: `Descrizione1` e completa nel DB ma `.product-info-name` la limita visualmente a due righe tramite max-height/overflow/`-webkit-line-clamp`; `PDP-TITLE-SHIPPING-INFO-1A` deve mostrare sempre il nome completo senza clamp o ellipsis. Nello stesso scope `div.shipping-to` deve smettere di duplicare Codice/EAN/Marca e usare soltanto informazioni spedizione reali KeepStore, con riferimento semantico a `Public/assets/keepstore/product-detail.html`.
- Prima del runtime verificare `SpedizioneGratis_Listini`, `SpedizioneGratis_Data_Inizio`, `SpedizioneGratis_Data_Fine`, listino e data correnti. Se validi, mostrare spedizione gratuita ed eventuale scadenza; altrimenti dichiarare che le spese sono calcolate nel carrello. Non inventare corriere, costo, tempi, consegna o destinazione; il campo `Peso` e la sua unita vanno verificati prima di esporli. Questo task non e full PDP parity, non include side cart e non cambia la priorita finale AI/Gemini/LLMS.
