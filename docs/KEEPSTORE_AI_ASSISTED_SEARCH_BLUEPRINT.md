...
- Stato typography trasversale: `GLOBAL-TYPOGRAPHY-ONSUS-AUDIT-1` e `GLOBAL-TYPOGRAPHY-ONSUS-1A` sono CHIUSI / A. L'audit runtime ha confermato che il repository non include Inter e che le dichiarazioni `"Inter", serif` possono risolvere Times New Roman; la foundation deterministica approvata al commit `d0f5f500f75da70aaf0c4a961762cf35f3db51ac` e `Arial, sans-serif`, senza modificare scale, pesi, line-height o layout ONSUS. `GLOBAL-TYPOGRAPHY-ONSUS-1B` e il prossimo task runtime per le sole famiglie esplicite residue; `1C` resta condizionale se computed style o smoke post-`1B` rilevano ancora serif accidentali o regressioni. Nessuna query, logica search/AI, ranking o runtime AI e stata modificata da audit/foundation typography.
