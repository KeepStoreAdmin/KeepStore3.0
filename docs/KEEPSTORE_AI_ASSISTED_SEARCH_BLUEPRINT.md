...
Nuova priorita prima delle evoluzioni brand/AI: `HOTFIX-STOREFRONT-PROMO-DEMO-BADGE-1A`, poi `STOREFRONT-OFFERS-PROMO-ONSUS-AUDIT-1A` e soltanto dopo `STOREFRONT-OFFERS-PROMO-UX-1A`; segue `PDP-BRAND-LOGO-1A`. `ProductCard.ascx` mantiene il finding aperto del testo promo hardcoded `Demo` in `phBadge`; non e una promozione con dati reali e non va considerata gestione promo completa. Catalogo e PDP restano non dichiarati completi; Gemini/LLMS resta alla fine.
...
- Nota storica: `LOGIN-RETURN-CONTEXT-1A` era il prossimo runtime ufficiale ed e ora chiuso / A, insieme a `LOGIN-ACCESS-AUDIT-1A`. La roadmap vigente parte dall'hotfix promo `Demo`, dall'audit promo ONSUS e dalla successiva UX promo; `PDP-BRAND-LOGO-1A` segue. Side cart, parity P1/P2 e blocchi ecommerce restano separati; SEO/AI/Gemini/LLMS rimane priorita finale. Catalogo e PDP non sono dichiarati completi.
