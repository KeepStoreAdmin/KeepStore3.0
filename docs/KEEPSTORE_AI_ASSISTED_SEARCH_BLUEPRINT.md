...
Checkpoint `MULTITENANT-BY-DESIGN`: fondamenta SEO, onboarding, Product structured data, asset runtime, prezzi/account e carrello sono integrate; il checkpoint stabile di base e `2d6ebd540a29ff906a01fa0820ebf65706150408`. `MULTI-STOREFRONT-ORDER-PROVENANCE-EMAIL-1A` estende ora lo stesso confine a ordine, replay, documenti e conferme e-mail; la PR #267 resta DRAFT in attesa dello smoke Product Owner sulla REV2 checkout. Search, AI, feed e structured data devono riusare identita tenant e dati autorevoli, mai inferire il cliente da host grezzi, nomi database, esempi merceologici o dati di un altro tenant.
...
Checkpoint corrente 2026-09-19: `frontend-rebuild` / `origin/frontend-rebuild` a `2d6ebd540a29ff906a01fa0820ebf65706150408`; `main` / `origin/main` invariati a `976e99f17cabc8a5c6a8715463444edfeaadcd91`. SEO, Product structured data, asset, account/prezzi e carrello multi-storefront sono integrati. Il task corrente certifica provenienza ordine, replay, visibilita documenti ed e-mail tenant-aware e corregge il submit checkout silenzioso senza abilitare dati ordine per search/AI/feed. Dopo chiusura e merge, il candidato immediato e `STOREFRONT-PERSISTENT-ANONYMOUS-CART-1A`, poi cart conversion UX, Merchant Center e abandoned-cart recovery subordinato a privacy/consenso; tutti `NON AVVIATO`.
...
Checkpoint cart rilevante ma non estensivo della roadmap AI: batch, retry, remove/clear, mobile UX, runtime recovery e `ORDER-DURABLE-IDEMPOTENCY-1A` sono chiusi e integrati; la PR #254 e stata incorporata fast-forward nel checkpoint `9fed039d4563fd20ecc02859236d90e7a2f0be5d`. Prezzi, promozioni, owner, listino e idempotenza restano server-side. Nessun task AI, feed, ranking, cache condivisa o retrieval puo mutare dati commerciali, ritentare mutazioni o assumere ownership. La chiusura non rende KeepStore `AI ready`, `SEO completo` o `Google pronto` e non modifica feed, JSON-LD, LLMS, Gemini o structured data.
