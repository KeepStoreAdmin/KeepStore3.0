...
Checkpoint corrente 2026-09-18: `frontend-rebuild` / `origin/frontend-rebuild` a `f337de11e4a97588289b865caf7dfdccd5089c1c`; `main` / `origin/main` invariati a `976e99f17cabc8a5c6a8715463444edfeaadcd91`. `GOOGLE-PRODUCT-STRUCTURED-DATA-1A` e chiuso A con PR #263. La REV2 asset runtime certifica due applicazioni/storefront sintetici nello stesso database logico; la REV3 rimuove il gradiente globale legacy e impedisce al moderno master di applicare automaticamente sfondi storici al `body`. Canonical/alias, seller, robots, sitemap, listini letti e asset restano legati alla medesima identita host-scoped. Product/Offer, prezzi e disponibilita non cambiano; il gate residuo e soltanto visuale e la PR #264 resta DRAFT fino alla conferma Product Owner. Il task successivo e `MULTI-STOREFRONT-COMMERCIAL-ISOLATION-1A`; Merchant Center resta sospeso. Le 46 immagini locali non tracciate restano escluse dai commit e preservate.
...
Stato promo/cart corrente: `HOTFIX-STOREFRONT-PROMO-DEMO-BADGE-1A`, `PROMO-COMMERCIAL-CORRECTNESS-1A`, `STOREFRONT-PROMO-ROUTES-HOTFIX-1A`, `PROMO-OWNER-ENFORCEMENT-1A REV2.7A`, `CART-BATCH-ATOMICITY-1A` e `CART-DEADLOCK-RETRY-1A` sono CHIUSI / A. Il carrello batch determina prezzi, promozioni, owner e listino server-side e conclude atomicamente tutte le righe oppure nessuna; il retry transazionale non estende questi dati a search o AI e non dichiara complete route promo, storefront, search, SEO o AI.

Contratto dati promo rilevante per search, crawler e futuri assistenti: una superficie pubblica puo esporre soltanto offerte pubbliche (`UtentiId IS NULL OR UtentiId <= 0`); una promo personale puo comparire esclusivamente nella sessione autenticata dell'owner corrispondente e non deve finire in output pubblico, cache condivise, feed, crawler o contesti AI. L'identita e i dati commerciali non provengono dal browser. HOME, catalogo e PDP devono riflettere la stessa risoluzione server-side usata da add/carrello/ordine: `ArticoliId + TCId`, fallback articolo autorizzato, minimo netto valido e tie-break deterministico per `OfferteDettaglioId`; stati/date/listino/owner invalidi e la combinazione ambigua `QntMinima + Multipli` falliscono chiusi. Un errore tecnico non equivale a "nessuna promo" e non autorizza la pubblicazione di un prezzo stimato.

La presentazione anonima usa valori `Decimal` reali: nessuna conversione da testo, `SafeDbl` o `Double`, nessuna percentuale fuori `1-99`, nessun badge per null/non numerico. Il flag `Prodotto_Gratis` indica spedizione gratuita, mai prodotto omaggio, ed e calcolato dal server con date e token listino esatto. Questi contratti sono prerequisiti per futuri feed, structured data, search assistita o AI; non autorizzano ancora JSON-LD dinamico, LLMS o cache di prezzi personali.
...
Roadmap vigente: parity, retirement, UX promo, HOME async cart, hardening error-state, bulk performance, fondamenta SEO, onboarding e Product structured data hanno completato il rispettivo perimetro. Canonical, robots, sitemap e dati strutturati PDP usano l'identita tenant autorevole; host grezzi/alterati e contaminazione cross-tenant falliscono chiusi. Gli asset tenant opzionali seguono la stessa regola: un file mancante o non sicuro viene omesso e non diventa una richiesta 404 o un fallback inventato. Dopo chiusura A e merge del task asset, il task obbligatorio e `MULTI-STOREFRONT-COMMERCIAL-ISOLATION-1A`, stato `NON AVVIATO`; `GOOGLE-MERCHANT-CENTER-FEED-1A` resta sospeso. `CART-IDEMPOTENCY-PERSISTENCE-AUDIT-1A` resta chiuso E e differito; Coupon/Groupon e recesso digitale restano differiti dal Product Owner.
...
Nota storica route promo: il precedente 500 nasceva da dati DB reinseriti in SQL dinamico con centinaia di `UNION ALL`; il hotfix `c088988080e54371a60c27c1f5979adf957358f1` lo rese set-based. I delta storici 25 solo moderni / 22 solo legacy sono superati dalla parita PR #255 (`SoloLegacy=0`, `SoloModern=0`, duplicati zero); la pagina query legacy e ora rimossa dal runtime dello shim. Structured data, JSON-LD, GTIN, feed e runtime AI non sono stati modificati.

Richiamo operativo, senza duplicare il root `AGENTS.md`: ChatGPT verifica Git/codice e applica la lettura proporzionata; questo blueprint e obbligatorio solo per search, AI, RAG, SEO AI o assistenza acquisto. Ordine: richiesta Germano; stato tecnico reale; `AGENTS.md`; checkpoint Masterplan; System Blueprint; questo blueprint quando pertinente; handoff/memoria non autoritativi. Gli SHA Git reali prevalgono: `d7734085...` e l'ultimo runtime stabile. Mobile-first: QA da `360px`/`390px`, niente A visuale senza browser mobile reale o funzioni essenziali hover-only. Codex esegue discovery e tutti i test/smoke tecnici e browser automatizzabili, pusha e apre/aggiorna la PR; ChatGPT controlla il diff e assegna A/B/E; Germano interviene solo per conferme umane non automatizzabili e autorizza il merge. Codex esegue il merge FF-only soltanto dopo entrambi i gate e completa sincronizzazione e verifiche finali; un blocco di strumenti, autenticazione o permessi produce B e non viene trasferito a Germano.
...
- eventuale ricerca di ordini e richieste di recesso deve essere strettamente owner-scoped e `AziendeId`-scoped; nessun dato ordine/recesso puo essere esposto ad altri utenti o domini;
- AI, ranking e ricerca non decidono automaticamente ammissibilita, eccezioni, decorrenza o accettazione legale del recesso; i casi incerti restano `Da verificare` da parte del processo autorizzato;
- dichiarazioni, recapiti, stato e contenuto del recesso non entrano in feed, JSON-LD, LLMS, structured data, cache pubbliche o retrieval pubblico;
- `B2C-WITHDRAWAL-DOCS-1A` non modifica runtime AI/search/SEO, endpoint, modelli, indicizzazione o dati.
...
Checkpoint runtime recovery: PR #252 chiusa A al checkpoint `055cb6e5c5d764222d5610e1b391d1c184f59882`. Guardrail per future superfici search/AI: nessun hook o fault injection deve essere raggiungibile nel runtime pubblicato; ogni componente globale deve caricare i propri stili ovunque venga renderizzato; i selettori JavaScript legacy generici vanno verificati prima di introdurre nuove classi UI. Nessuna logica search, ranking, feed o AI e stata modificata.
