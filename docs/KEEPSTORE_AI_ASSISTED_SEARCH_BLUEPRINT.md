...
Checkpoint corrente 2026-09-17: `frontend-rebuild` / `origin/frontend-rebuild` a `23f21e6da00e726db9e361ace68de39b3d8f77eb`; `main` / `origin/main` invariati a `976e99f17cabc8a5c6a8715463444edfeaadcd91`. `PROMO-DISPLAY-ERROR-STATE-HARDENING-1A` e chiuso A con PR #259 integrata fast-forward. `STOREFRONT-PROMO-BULK-PERFORMANCE-1A` e il task corrente; misura e riduce esclusivamente lavoro promo duplicato request-scoped. JavaScript, search e AI non sono fonte di verita per owner, quantita, prezzo o idempotenza. Le 46 immagini locali non tracciate restano escluse dai commit e preservate.
...
Roadmap vigente: parity, retirement, UX promo, HOME async cart e hardening error-state hanno completato il rispettivo perimetro; PR #259 e integrata. La baseline performance corrente certifica una query/snapshot per contesto e nessun N+1 SQL; la sola HOME evita la risoluzione dei pool ordinari scartati (`948/617/331` chiamate/uniche/duplicate prima, `438/438/0` dopo), senza cache condivise e senza memorizzare `TechnicalError`. Il candidato successivo `STOREFRONT-SEO-TECHNICAL-AUDIT-1A` verifichera canonical, robots, sitemap, meta, paginazione, filtri e indicizzabilita dopo chiusura A del task corrente; stato `NON AVVIATO`. `CART-IDEMPOTENCY-PERSISTENCE-AUDIT-1A` resta chiuso E e differito; Coupon/Groupon e recesso digitale restano differiti dal Product Owner. Google Product structured data e AI/Gemini/LLMS restano finali; feed, JSON-LD, LLMS, Gemini e structured data non sono stati modificati. HOME, catalogo, PDP e promo non sono dichiarati completi.
