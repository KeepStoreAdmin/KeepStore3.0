...
Stato promo corrente: `HOTFIX-STOREFRONT-PROMO-DEMO-BADGE-1A`, `PROMO-COMMERCIAL-CORRECTNESS-1A`, `STOREFRONT-PROMO-ROUTES-HOTFIX-1A` e `PROMO-OWNER-ENFORCEMENT-1A REV2.7A` sono CHIUSI / A con smoke Germano A. L'ultimo task garantisce owner scope e autorita server su promo, quantita, prezzi e free-shipping; non dichiara complete le route promo o lo storefront.
...
Roadmap vigente: i debiti tecnici successivi al contratto owner-scoped restano task separati (`CART-BATCH-ATOMICITY-1A`, retry deadlock/race DB, mobile cart UX, HOME TTFB, audit dipendenze MySQL e monetari `DOUBLE`). Parity/retirement delle route promo richiede ancora prova dedicata. Coupon/Groupon resta differito a `COUPON-CART-LIFECYCLE-1A` e non va attivato senza richiesta esplicita Germano. SEO tecnico, Google Product structured data e AI/Gemini/LLMS restano finali; `LLMS-TXT-ASHX-JSONLD-AUDIT-1` e ancora solo futuro read-only. HOME, catalogo, PDP e promo non sono dichiarati completi.
...
Richiamo operativo, senza duplicare il root `AGENTS.md`: prima dei prompt importanti ChatGPT verifica Git/codice reale e applica la lettura documentale proporzionata. Questo blueprint e obbligatorio solo per task search, AI, RAG, SEO AI o assistenza acquisto; la lettura integrale delle fonti e riservata ad audit ampi e riconciliazioni trasversali. Ordine: richiesta approvata da Germano; stato tecnico reale; `AGENTS.md`; checkpoint Masterplan; System Blueprint; questo blueprint quando pertinente; handoff e memoria non autoritativi. Gli SHA Git reali prevalgono: il checkpoint `30928ee8...` sopra identifica l'ultimo runtime stabile, mentre il base documentale verificato di questo task e `b0241ac4fdb843c6b48421c4bdc6fccb2e2a04e7`. Il mobile e esperienza primaria: QA da `360px`/`390px`, nessun A visuale senza browser mobile reale, nessuna funzione essenziale hover-only. Codex esegue test strumentali e discovery proattiva ma propone senza ampliare il manifest; ChatGPT decide; Germano verifica resa commerciale e percorsi utente semplici. Branch runtime/docs sempre pushati prima del report, un solo task attivo e classificazione A/B/E secondo `AGENTS.md`.
