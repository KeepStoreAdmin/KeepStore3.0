...
11. `CATALOG-MULTISELECT-ADD-BUTTON-1A`
   ...
   - restano backlog separati side cart/offcanvas, ritorno `Continua lo shopping`, quantita gia presente nel carrello su `articolo.aspx`, funzioni ONSUS mancanti e immagini rotte/path DB.

12. `CART-QTY-IN-CATALOG-1A`
   ...
   - smoke mobile reale ha aperto backlog separati `BUG-ADD-TO-CART-ANON-MOBILE-LOGIN-MESSAGE` e `BUG-CART-QTY-MOBILE-VISUAL`; non dichiarare mobile OK finche i percorsi reali non sono verificati e corretti.

13. `AI-ASSISTANT-DATA-PROFILE-AUDIT-1A`
