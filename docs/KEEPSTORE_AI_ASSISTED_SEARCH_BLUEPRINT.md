...
Checkpoint corrente 2026-09-10: `frontend-rebuild` / `origin/frontend-rebuild` a `d773408588c4fc91472a4b71ef9d3980e1c93072`; `main` / `origin/main` invariati a `976e99f17cabc8a5c6a8715463444edfeaadcd91`. `WORKFLOW-GOVERNANCE-1A`, `CART-BATCH-ATOMICITY-1A` e `CART-DEADLOCK-RETRY-1A` sono chiusi e integrati; PR #245, fast-forward dalla base `ef3bce60af997b35a2779f43a102c2df5c4ae7c7`, un commit e zero merge commit. Le directory non tracciate `Public/assets/images/articoli/`, `Public/assets/images/marche/` e `Public/assets/images/settori/` restano escluse dai commit. Anche eventuali loghi locali non tracciati sotto `Public/assets/images/vettori/` vanno preservati e possono essere aggiunti solo se nominativamente autorizzati da uno specifico manifest.
...
Roadmap vigente: `CART-BATCH-ATOMICITY-1A` e `CART-DEADLOCK-RETRY-1A` non sono piu debiti aperti. Il prossimo cart task raccomandato e `CART-REMOVE-TRANSACTION-HARDENING-1A`, seguito da `CART-IDEMPOTENCY-PERSISTENCE-AUDIT-1A`, mobile cart UX, touch target rimozione `44x44` e audit monetari `DOUBLE`; HOME TTFB e audit dipendenze MySQL restano separati. Parity/retirement delle route promo richiede ancora prova dedicata. Coupon/Groupon resta differito a `COUPON-CART-LIFECYCLE-1A` e non va attivato senza richiesta esplicita Germano. SEO tecnico, Google Product structured data e AI/Gemini/LLMS restano finali; `LLMS-TXT-ASHX-JSONLD-AUDIT-1` e ancora solo futuro read-only. Feed, JSON-LD, LLMS, Gemini e structured data non sono stati modificati. HOME, catalogo, PDP e promo non sono dichiarati completi.
...
Contratto route promo rilevante per search/SEO/AI: il precedente 500 nasceva da dati DB reinseriti in SQL dinamico con centinaia di `UNION ALL`; apostrofo e successivo `@CLIP` rompevano la query. Il hotfix `c088988080e54371a60c27c1f5979adf957358f1` usa una query set-based fissa tra `vOfferteDettagli` e `varticolilistini`, colonne esplicite, ordine deterministico e 11 parametri tipizzati. Parita verificata 935/935 con multiset diff zero; `pid=6534`, `14556` e `99999999` rendono rispettivamente 1, 2 e 0 righe con HTTP 200. Questo risolve la rottura/second-order SQL injection della route, non la parita moderna: 25 prodotti restano solo moderni e 22 solo legacy, con `pid`, `UtentiID`, quantita minima/multipli e filtri da riconciliare. Structured data, JSON-LD, GTIN, feed, sitemap e runtime AI non sono stati modificati.
...
Checkpoint cart rilevante ma non estensivo della roadmap AI: `CART-BATCH-ATOMICITY-1A` e `CART-DEADLOCK-RETRY-1A` sono chiusi; prezzi, promozioni, owner, listino, idempotenza e decisioni di retry restano server-side. Il prossimo debito cart e `CART-REMOVE-TRANSACTION-HARDENING-1A`, non un task AI. Retry cart e idempotenza non autorizzano AI, feed, ranking, cache condivise o retrieval a mutare dati commerciali, ritentare mutazioni o assumere ownership. La chiusura non rende KeepStore `AI ready`, `SEO completo` o `Google pronto` e non modifica feed, JSON-LD, LLMS, Gemini o structured data.
