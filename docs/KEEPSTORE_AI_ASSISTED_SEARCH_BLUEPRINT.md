...
`MULTIPROVIDER-TENANT-EMAIL-ALL-CALLERS-MIGRATION-1A-REV1` completa il checkpoint applicativo della PR #268: ordine/BCC, registrazione/profilo, reset password tokenizzato e contatti moderno/legacy usano un solo facade tenant-scoped con `Purpose=TRANSACTIONAL`; le code `INVIADOCUMENTI` restano un confine esterno dichiarato. Nessun chiamante legge SMTP legacy o conserva credenziali in Session. Il runner amministrativo locale riusa il runtime canonico per TLS/AUTH/QUIT senza messaggio, endpoint pubblico o aggiornamento DB e restituisce solo esiti sanitizzati. Nel solo ambiente locale `taikun` i due profili risultano verificati e attivi con credenziali DPAPI fuori database/repository; nessuna e-mail reale o prova end-to-end di consegna e stata eseguita. La produzione non e configurata e deve ripetere migration/verifica, provisioning, ACL, nuovi riferimenti, `VerifyConnection` e attivazione. La rotazione delle credenziali gia esposte e differita e resta debito di sicurezza del futuro pannello gestionale; OAuth2 Google/Microsoft resta `NON OPERATIVO`.
