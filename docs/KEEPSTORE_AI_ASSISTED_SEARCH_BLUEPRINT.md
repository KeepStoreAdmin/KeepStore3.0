...
Checkpoint `MULTITENANT-BY-DESIGN`: le fondamenta SEO tenant-aware e il contratto onboarding sono integrati con PR #261/#262; il checkpoint stabile e `07522fa332c98b83bd535abd090d0d8899ee262a`. `GOOGLE-PRODUCT-STRUCTURED-DATA-1A` e il task corrente e introduce sulla sola PDP il primo modulo deterministico del futuro SEO Automation Engine. Search, AI, feed e structured data devono riusare identita tenant e dati commerciali autorevoli, mai inferire il cliente da host grezzi, nomi database, esempi merceologici o dati di un altro tenant.
...
Checkpoint corrente 2026-09-18: `frontend-rebuild` / `origin/frontend-rebuild` a `07522fa332c98b83bd535abd090d0d8899ee262a`; `main` / `origin/main` invariati a `976e99f17cabc8a5c6a8715463444edfeaadcd91`. `STOREFRONT-SEO-TECHNICAL-AUDIT-1A` e `KEEPSTORE-MULTITENANT-ONBOARDING-CONTRACT-1A` sono chiusi; `GOOGLE-PRODUCT-STRUCTURED-DATA-1A` e in corso. Il runtime anonimo e i test isolati sono positivi, ma lo smoke HTTP autenticato PROVA resta un gate: esito complessivo `E` e PR `DRAFT` finche non viene completato in una sessione sicura. JavaScript, search e AI non sono fonte di verita per tenant, owner, quantita, prezzo o idempotenza. Le 46 immagini locali non tracciate restano escluse dai commit e preservate.
...
Roadmap vigente: parity, retirement, UX promo, HOME async cart, hardening error-state, bulk performance, fondamenta SEO e onboarding hanno completato il rispettivo perimetro. Canonical, robots, sitemap e dati strutturati PDP usano l'identita tenant autorevole; host grezzi/alterati e contaminazione cross-tenant falliscono chiusi. Il modulo Product riusa lo snapshot commerciale visibile, espone il prezzo applicabile alla quantita `1`, omette tier futuri come prezzo principale e non inventa varianti, rating, resi o immagini. Dopo review A e merge di `GOOGLE-PRODUCT-STRUCTURED-DATA-1A`, il candidato e `GOOGLE-MERCHANT-CENTER-FEED-1A`, stato `NON AVVIATO`. `CART-IDEMPOTENCY-PERSISTENCE-AUDIT-1A` resta chiuso E e differito; Coupon/Groupon e recesso digitale restano differiti dal Product Owner.
