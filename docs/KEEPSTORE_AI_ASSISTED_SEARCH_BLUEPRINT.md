...
Checkpoint `MULTITENANT-BY-DESIGN`: fondamenta SEO, onboarding, Product structured data, asset runtime, prezzi/account e carrello sono integrate; il checkpoint stabile di base e `2d6ebd540a29ff906a01fa0820ebf65706150408`. `MULTI-STOREFRONT-ORDER-PROVENANCE-EMAIL-1A` estende ora lo stesso confine a ordine, replay, documenti e conferme e-mail. Search, AI, feed e structured data devono riusare identita tenant e dati autorevoli, mai inferire il cliente da host grezzi, nomi database, esempi merceologici o dati di un altro tenant.
...
Checkpoint storico cart: `CART-REMOVE-TRANSACTION-HARDENING-1A`, mobile UX, runtime recovery e `ORDER-DURABLE-IDEMPOTENCY-1A` sono chiusi A e integrati. Il checkpoint operativo vigente e quello iniziale del documento; idempotenza cart persistente resta E/differita. Recesso digitale e Coupon/Groupon restano differiti.
...
Guardrail AI/search: dati, cache o retrieval futuri non possono usare il solo `LoginId`, `SessionID`, database o articolo come confine; devono includere database sanitizzato, azienda, listino, stato autenticato e owner, senza esporre token o identificativi. Ordini, documenti, ricevute e contenuti e-mail sono inoltre vincolati al `documenti.AziendeId` persistito; non diventano input AI/RAG o feed. Il prossimo task `GOOGLE-MERCHANT-CENTER-FEED-1A` resta `NON AVVIATO` fino a review e merge del checkpoint ordine.
