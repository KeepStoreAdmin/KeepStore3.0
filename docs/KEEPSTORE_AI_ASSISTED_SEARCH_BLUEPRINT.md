...
- Priorita trasversale prima della quantita PDP: `GLOBAL-TYPOGRAPHY-ONSUS-AUDIT-1`, poi rollout `GLOBAL-TYPOGRAPHY-ONSUS-1A/1B/...`. L'audit include `Default.aspx`/HOME e renderer/card inline, `articoli.aspx`, `articolo.aspx`, `carrello.aspx` come riferimento visuale, `Page.master`, SiteHeader, SiteFooter, MiniCart, Quick View, Compare, Breadcrumb e pagine pubbliche connesse. Deve misurare computed style/font risolto desktop-mobile: `font.css` locale non risulta definire Inter, mentre varie superfici usano `"Inter", serif` e il carrello usa `"Inter", Arial, sans-serif`. Il riferimento carrello non autorizza dimensioni uniformi: gerarchia ONSUS per titoli, body, prezzi, metadata, caption, CTA e card resta vincolante.
