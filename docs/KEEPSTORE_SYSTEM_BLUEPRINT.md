...
12. **Germano non e il programmatore.** ChatGPT e Codex eseguono i controlli tecnici ragionevolmente automatizzabili e non scaricano su Germano analisi codice, diff, query, stack trace, debugging o ricerca dipendenze. A Germano spettano soprattutto smoke reali mirati, resa commerciale e visuale, mobile reale, percorsi utente e valutazioni che richiedono runtime o giudizio umano.
13. **Test ownership.** Codex cura build/precompile quando pertinenti, analisi statica disponibile, query read-only autorizzate, test tecnici, diff, dipendenze, regression check, sanity e discovery collegata. ChatGPT definisce i test, verifica commit/diff, valuta finding ed estensioni, controlla le evidenze e assegna A/B/E senza accettare A insufficiente. Germano esegue lo smoke reale mirato e valuta resa commerciale, visuale, mobile reale e flussi utente.
...
16. **Definition of Done runtime.** Un task runtime e proponibile A solo con requisito autorizzato implementato, causa affrontata quando pertinente, manifest rispettato, baseline di test eseguita, regression check mirati superati, nessun blocker o regressione nota nel perimetro, finding extra riportati ma non implementati senza autorizzazione, diff proporzionato, branch pushato ed evidenze sufficienti per la review ChatGPT. "Funziona nel caso mostrato" non equivale da solo ad A; un micro-task A non dichiara completa la pagina o l'area.
...
20. **Compatibilita e continuita.** Restano vincolanti micro-task, un solo task attivo, manifest, branch separato, base SHA verificata, `main` protetto, stack WebForms/VB.NET/.NET Framework/MySQL, mobile-first, sicurezza, push origin, review ChatGPT, classificazione A/B/E, smoke Germano, merge FF-only, no false closure e Git come source of truth.
...
Stato Git verificato prima di `WORKFLOW-GOVERNANCE-1A`: HEAD documentale `frontend-rebuild` / `origin/frontend-rebuild` `b0241ac4fdb843c6b48421c4bdc6fccb2e2a04e7`; ultimo runtime stabile `30928ee81b9bbdc23b55c5aaa057ef1bf4b5d3ef`; `main` / `origin/main` a `976e99f17cabc8a5c6a8715463444edfeaadcd91`. `PROMO-OWNER-ENFORCEMENT-1A REV2.7A` e integrato fast-forward only, senza merge commit. Working tree atteso: soltanto asset immagini locali non tracciati e preservati. Le directory `Public/assets/images/articoli/`, `Public/assets/images/marche/` e `Public/assets/images/settori/` restano escluse dai commit; `Public/assets/images/vettori/` puo contenere altri loghi locali non tracciati, da non aggiungere in blocco e autorizzabili solo nominativamente nel manifest di un task. Gli SHA precedenti restano riferimenti storici corretti, non descrizioni della stable corrente.
...
- Debiti non chiusi: `CART-BATCH-ATOMICITY-1A`, retry deadlock/race DB, `CART-MOBILE-RESPONSIVE-UX-1A`, target remove `44x44`, `HOME-TTFB-PERFORMANCE-1A`, `MYSQL-CONNECTOR-DEPENDENCY-AUDIT-1A` e audit monetari `DOUBLE`. Coupon/Groupon non e certificato: `COUPON-CART-LIFECYCLE-1A` resta differito e inattivo finche Germano non lo richiede; nessun percorso GET legacy va documentato come operativo.
