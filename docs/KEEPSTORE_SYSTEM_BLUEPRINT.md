...
Checkpoint Git corrente 2026-09-10: `frontend-rebuild` / `origin/frontend-rebuild` a `d773408588c4fc91472a4b71ef9d3980e1c93072`; `main` / `origin/main` a `976e99f17cabc8a5c6a8715463444edfeaadcd91`. `WORKFLOW-GOVERNANCE-1A`, `CART-BATCH-ATOMICITY-1A` e `CART-DEADLOCK-RETRY-1A` sono chiusi e integrati. La PR #245 ha applicato un fast-forward dalla base `ef3bce60af997b35a2779f43a102c2df5c4ae7c7` con un commit e zero merge commit. Working tree atteso: soltanto asset immagini locali non tracciati e preservati nelle directory autorizzate. Gli SHA precedenti restano riferimenti storici corretti, non descrizioni della stable corrente.
...
- Debiti non chiusi: `CART-REMOVE-TRANSACTION-HARDENING-1A`, `CART-IDEMPOTENCY-PERSISTENCE-AUDIT-1A`, `CART-MOBILE-RESPONSIVE-UX-1A`, target rimozione `44x44`, `HOME-TTFB-PERFORMANCE-1A`, `MYSQL-CONNECTOR-DEPENDENCY-AUDIT-1A` e audit monetari `DOUBLE`. `CART-BATCH-ATOMICITY-1A` e `CART-DEADLOCK-RETRY-1A` sono chiusi. Coupon/Groupon non e certificato: `COUPON-CART-LIFECYCLE-1A` resta differito e inattivo finche Germano non lo richiede; nessun percorso GET legacy va documentato come operativo.
...
- Il successivo debito cart e `CART-REMOVE-TRANSACTION-HARDENING-1A`, priorita HIGH e audit-first: rimozione singola e svuotamento sono ancora fuori dalle transazioni canoniche e richiedono verifica di owner scope, atomicita e ordine lock prima di un fix.
...
- Confini ancora aperti: `CART-REMOVE-TRANSACTION-HARDENING-1A` HIGH e prossimo audit consigliato; `CART-IDEMPOTENCY-PERSISTENCE-AUDIT-1A`; `ORDER-DOCUMENT-INVENTORY-LOCK-AUDIT-1A`; `PROMO-AMBIGUOUS-STATE-REACHABILITY-1A`. Il recesso digitale resta documentato ma differito per decisione del Product Owner.
...
- `CART-REMOVE-TRANSACTION-HARDENING-1A`: priorita HIGH e prossimo audit consigliato. Rimozione singola e svuotamento restano fuori dalle transazioni canoniche; verificare owner scope, atomicita e ordine lock prima del fix.
...
- Ordine cart successivo: `CART-REMOVE-TRANSACTION-HARDENING-1A`, `CART-IDEMPOTENCY-PERSISTENCE-AUDIT-1A`, `CART-MOBILE-RESPONSIVE-UX-1A`, touch target rimozione `44x44`, audit schema monetario `DOUBLE`. Mantenere separati `HOME-TTFB-PERFORMANCE-1A` e `MYSQL-CONNECTOR-DEPENDENCY-AUDIT-1A`.
- `ORDER-DOCUMENT-INVENTORY-LOCK-AUDIT-1A`: task separato su ordine del cursore carrello, inventario, numerazione documento e commit ambiguo; non implementato dal retry cart.
