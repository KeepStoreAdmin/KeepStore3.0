...
- `STOREFRONT-AVAILABILITY-PRESENTATION-1A` e integrato in `frontend-rebuild` con fast-forward al merge point `b78cbedab9cd21806cd57d7efb0d91e8c0a8d0f3`. L'architettura finale e descritta in `5.2.2`; `CATALOG-ASYNC-CART-1A` e stato poi chiuso / A e il prossimo runtime corrente e `LOGIN-RETURN-CONTEXT-1A`.
...
- Roadmap: prossimo runtime `LOGIN-RETURN-CONTEXT-1A`, iniziando con audit mirato del flusso login e confronto ecommerce moderno; preservare in sicurezza la provenienza oppure scegliere una destinazione commerciale sicura, senza implementazione anticipata. Seguono `PDP-BRAND-LOGO-1A` e `STOREFRONT-OFFERS-PROMO-VERIFY-1A` solo con dati promo reali attivati. AI/Gemini/LLMS resta finale e separato. Catalogo e PDP non sono dichiarati completi.
...
- `GLOBAL-TYPOGRAPHY-ONSUS-1C` e NON NECESSARIO / NON CREATO dopo esito A di `1B`; il blocco typography storefront e CHIUSO. La chiusura non dichiara complete HOME, catalogo, PDP, header, footer o l'area ecommerce. Catalog audit e runtime `1A/1B/1C`, availability e `CATALOG-ASYNC-CART-1A` sono stati eseguiti; il prossimo runtime e `LOGIN-RETURN-CONTEXT-1A`, mentre side cart resta successivo e separato. Il vecchio `ARTICOLO-CART-QTY-IN-PDP-AUDIT-1` non e piu successivo: audit e runtime quantita PDP sono assorbiti dal cart-state storefront globale.
...
- `CATALOG-ONSUS-PARITY-1C-IMAGE-PATH-404`: CHIUSO / A; anche pager/posizione recent, quattro viste mobile, compact grid e containment recent sono chiusi dal blocco runtime successivo. I gap P1/P2 catalogo residui restano backlog. Availability e `CATALOG-ASYNC-CART-1A` sono integrati; l'ordine operativo corrente e `LOGIN-RETURN-CONTEXT-1A`, `PDP-BRAND-LOGO-1A`, quindi `STOREFRONT-OFFERS-PROMO-VERIFY-1A` soltanto quando i dati promo saranno attivi. `SIDE-CART-OFFCANVAS-ONSUS-AUDIT-1` resta separato.
