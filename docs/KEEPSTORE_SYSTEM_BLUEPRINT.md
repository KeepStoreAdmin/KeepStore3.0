...
Stato Git prima del merge runtime `STOREAVAIL-1A`: `frontend-rebuild` / `origin/frontend-rebuild` a `b31f8177e09cffcd5e9193e1e26eb3bbec4f8e6b`; branch `task/storefront-availability-presentation-1a` funzionalmente accettato a HEAD applicativo `2b4e751d0041aa6ee4ec5be26a1cd138c3b3bc71`; `main` / `origin/main` a `976e99f17cabc8a5c6a8715463444edfeaadcd91`. Gli SHA catalog parity, typography e product-image precedenti restano riferimenti storici corretti, non descrizioni della stable corrente.
...
- `STOREFRONT-AVAILABILITY-PRESENTATION-1A` e funzionalmente accettato sul branch task a HEAD applicativo `2b4e751d0041aa6ee4ec5be26a1cd138c3b3bc71`; il consolidamento su `frontend-rebuild` resta un merge separato. L'architettura finale e descritta in `5.2.2`.
...
- `GLOBAL-TYPOGRAPHY-ONSUS-1C` e NON NECESSARIO / NON CREATO dopo esito A di `1B`; il blocco typography storefront e CHIUSO. La chiusura non dichiara complete HOME, catalogo, PDP, header, footer o l'area ecommerce. Catalog audit e runtime `1A/1B/1C` sono poi stati eseguiti; `STOREFRONT-AVAILABILITY-PRESENTATION-1A` e ora accettato funzionalmente e in attesa del merge separato, mentre side cart resta successivo e separato. Il vecchio `ARTICOLO-CART-QTY-IN-PDP-AUDIT-1` non e piu successivo: audit e runtime quantita PDP sono assorbiti dal cart-state storefront globale.
...
- `STOREFRONT-AVAILABILITY-PRESENTATION-1A` e funzionalmente accettato sul branch `task/storefront-availability-presentation-1a`, base `a5eb1c5823997b83430cd49874c794a9d1bfa18e`, HEAD applicativo pre-docs `2b4e751d0041aa6ee4ec5be26a1cd138c3b3bc71`. `main` / `origin/main` restano `976e99f17cabc8a5c6a8715463444edfeaadcd91`; questa registrazione non equivale al merge runtime.
...
- `CATALOG-ASYNC-CART-1A` resta futuro e separato: il flusso preesistente `.js-ks-cart-link` usa `preventDefault()` + `window.location.assign()` verso `cart_add.aspx`, che delega ad `aggiungi.aspx` e poi ridirige al catalogo con render WebForms completo. `#ksMiniCartCanvas` e server-rendered da `MiniCart.ascx.vb`; manca un contratto async/fragment sicuro riusabile. Il futuro intervento deve riusare la business logic server, aggiornare mini-cart/count/card, impedire doppio submit, preservare stato e scroll delle quattro viste e coprire anonimo, autenticato e multiselect: non e un semplice fix JavaScript.
...
- `CATALOG-ONSUS-PARITY-1C-IMAGE-PATH-404`: CHIUSO / A; anche pager/posizione recent, quattro viste mobile, compact grid e containment recent sono chiusi dal blocco runtime successivo. I gap P1/P2 catalogo residui restano backlog. `STOREFRONT-AVAILABILITY-PRESENTATION-1A` e accettato funzionalmente e in attesa del merge separato; dopo il merge l'ordine operativo e `CATALOG-ASYNC-CART-1A`, `PDP-BRAND-LOGO-1A`, quindi `STOREFRONT-OFFERS-PROMO-VERIFY-1A` soltanto quando i dati promo saranno attivi. `SIDE-CART-OFFCANVAS-ONSUS-AUDIT-1` resta separato.
