...
- Roadmap vigente: `STOREFRONT-PROMO-ROUTES-AUDIT-1A`; eventuale hotfix solo se `promozioni.aspx` e attiva, altrimenti decisione separata redirect/canonical/noindex; `STOREFRONT-OFFERS-PROMO-UX-1A`; `HOME-ASYNC-CART-1A`; `PDP-BRAND-LOGO-1A`; poi performance, SEO tecnico, Google Product structured data e infine AI/Gemini/LLMS. `promozioni.aspx` HTTP 500, `articolix.aspx` 500 legacy `TCid`, pagine protette viste solo come 302, materializzazione promo e bulk/N+1 restano finding separati. HOME, catalogo, PDP e redesign promo complessivo non sono dichiarati completi.
...
- Roadmap storica: `LOGIN-RETURN-CONTEXT-1A` era il successivo audit/implementation del flusso login; e stato chiuso / A, insieme a `LOGIN-ACCESS-AUDIT-1A`. L'ordine vigente parte dall'hotfix `Demo` promo ed e definito in `2.3.1`; catalogo e PDP non sono dichiarati completi.
