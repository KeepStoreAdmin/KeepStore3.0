...
Esempio vincolante: `articoli.aspx` ha micro-task chiusi su card prodotto, selezione multipla, quantita gia nel carrello, delta quantita, visual mobile della quantita e CTA `Acquista`. Search/suggest, zero-results, sidebar/facet e filtri applicati hanno interventi documentati per perimetri specifici, ma non vanno dichiarati completi come funzioni pagina senza audit dedicato. Questo non rende la pagina catalogo completa ne "allineata ONSUS" in senso pieno. Restano da auditare e implementare full ONSUS parity, funzioni residue, controlli, filtri, sidebar, sort, layout, componenti commerciali e comportamento responsive complessivo con task dedicati.
...
Stato Git stabile corrente dopo `GLOBAL-TYPOGRAPHY-ONSUS-1B`: `frontend-rebuild` / `origin/frontend-rebuild` a `df98e70ced14a6f635d7819fadebe22d09f103ee`; `main` / `origin/main` a `976e99f17cabc8a5c6a8715463444edfeaadcd91`. Il masterplan operativo contiene la scaletta prioritaria ufficiale; questo blueprint deve restare coerente e non anticipare come completati backlog ancora aperti.
...
- `GLOBAL-TYPOGRAPHY-ONSUS-AUDIT-1`, `GLOBAL-TYPOGRAPHY-ONSUS-1A` e `GLOBAL-TYPOGRAPHY-ONSUS-1B`: CHIUSI / A. `GLOBAL-TYPOGRAPHY-ONSUS-1C`: NON NECESSARIO / NON CREATO. Il blocco typography storefront e chiuso; prossimo runtime ufficiale `CATALOG-ONSUS-PARITY-AUDIT-1`, senza dichiarare `articoli.aspx` completa.
