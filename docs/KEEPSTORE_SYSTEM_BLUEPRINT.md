...
Per la scheda prodotto, `Disponibilita` visualizzata non deve dipendere dal campo aggregato quando questo e zero ma `Giacenza` e positiva: il display numerico deve preservare la semantica legacy `Giacenza` come disponibilita esposta, `Impegnata` come impegnati e `InOrdine` come arrivi. Le offerte articolo di dettaglio possono essere renderizzate tramite helper read-only dedicato da `voffertedettagli`, senza immagini legacy `Public/Images/`, senza modificare il calcolo carrello/add-to-cart e senza introdurre logiche nuove di prezzo ordine.
