...
- Residui intenzionali aperti per `GLOBAL-TYPOGRAPHY-ONSUS-1B`: card catalogo `#ksCatalogPage article.card-product`, link settore/categoria/tipologia del MegaMenu, `.font-main`, `.font-2`, `.ft-heading`, caption/CTA e classi HOME con famiglia esplicita, footer heading e MiniCart/Breadcrumb/Quick View/Compare solo se il runtime mostra ancora serif esplicito. Il PDP principale e gia Arial e va controllato solo per regressioni.
- `1B` deve normalizzare esclusivamente le famiglie residue nel layer KeepStore scoped, senza mass edit del CSS base ONSUS. `GLOBAL-TYPOGRAPHY-ONSUS-1C` si apre soltanto se CDP o smoke Germano post-`1B` rilevano Times/Poppins/serif accidentali, overflow o regressioni; diversamente il blocco typography si chiude dopo `1B`. Il vecchio `ARTICOLO-CART-QTY-IN-PDP-AUDIT-1` non e piu successivo: audit e runtime quantita PDP sono assorbiti dal cart-state storefront globale.
...
- `GLOBAL-TYPOGRAPHY-ONSUS-AUDIT-1` e `GLOBAL-TYPOGRAPHY-ONSUS-1A`: CHIUSI / A. Prossimo task runtime `GLOBAL-TYPOGRAPHY-ONSUS-1B`, limitato alle famiglie esplicite residue; `1C` solo se necessario dopo computed-style audit e smoke.
