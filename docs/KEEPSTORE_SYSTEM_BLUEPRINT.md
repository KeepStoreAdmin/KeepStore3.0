...
- Prima dei prompt importanti ChatGPT legge i tre manuali, confronta runtime e documentazione e, dopo la pubblicazione del branch, controlla direttamente commit/diff GitHub per la review A/B/E.
...
2. **Responsabilita di ChatGPT.** Prima di ogni prompt Codex, ChatGPT legge i tre manuali, verifica Git/repository e requisito funzionale/commerciale, identifica superficie e dipendenze plausibili, valuta regressioni, sicurezza e impatto commerciale, stabilisce test, manifest, stop rule e profilo di esecuzione. ChatGPT e il coordinatore tecnico e decisionale; Codex e esecutore tecnico, analista e tester, ma non decide ampliamenti o migliorie extra.
3. **Profili di esecuzione.** ChatGPT assegna un solo profilo, scegliendo il meno costoso compatibile con una elevata probabilita di A al primo passaggio, senza preferire automaticamente il livello piu economico:
   - `ECONOMY`: documentazione semplice, modifica meccanica, ricerca mirata o cambio localizzato a rischio molto basso.
   - `STANDARD`: sviluppo o bugfix circoscritto, UI/backend con dipendenze identificabili, normale WebForms/VB.NET.
   - `DEEP`: piu componenti, query/DB, sessione, frontend e backend, sicurezza, comportamento commerciale o rischio regressione significativo.
   - `CRITICAL`: prezzi, listini, promo con impatto economico, carrello, autenticazione, dati utente, checkout, ordine, pagamento, redirect sensibile o modifica trasversale ad alto impatto.
4. **Modello e reasoning.** Il profilo guida modello consigliato, reasoning, profondita dell'analisi e ampiezza delle verifiche, nei limiti delle opzioni realmente disponibili nell'ambiente Codex/IDE/Work. Il testo del prompt non cambia automaticamente il modello. Codex non abbassa autonomamente il profilo; se lo ritiene insufficiente segnala `ESCALATION REQUIRED` e si ferma prima di ampliare sostanzialmente il lavoro.
...
6. **Proactive engineering review mirata.** Nel task runtime Codex analizza la superficie effettivamente coinvolta e le dipendenze reali per trovare e segnalare bug collegati, regressioni potenziali, casi limite, duplicazioni pericolose, codice morto interferente, problemi di sicurezza/validazione, incoerenze frontend-backend o commerciali, errori di stato/sessione, problemi mobile/accessibilita pertinenti, query inefficienti direttamente coinvolte, comportamento legacy interferente, helper riutilizzabili e semplificazioni sicure. Questa ricerca non diventa un audit generale del repository.
...
9. **No scope creep.** Essere proattivi significa vedere e segnalare, non modificare autonomamente tutto cio che emerge. Regola vincolante: **CODEX ANALIZZA + CODEX TESTA + CODEX SEGNALA; CHATGPT DECIDE; CODEX IMPLEMENTA SOLO CIO CHE E AUTORIZZATO.**
...
17. **Stop conditions.** Codex si ferma con B, motivo ed evidenza, invece di improvvisare quando servono schema DB non autorizzato, nuove librerie, cambio architetturale, chiarimento commerciale, mitigazione di sicurezza fuori scope, file indispensabili fuori manifest, violazione di un contratto chiuso, finding `BLOCKER` non autorizzato o una verifica critica non eseguibile.
18. **Compatibilita e continuita.** Restano vincolanti micro-task, un solo task attivo, manifest, branch separato, base SHA verificata, `main` protetto, stack WebForms/VB.NET/.NET Framework/MySQL, mobile-first, sicurezza, push origin, review ChatGPT, classificazione A/B/E, smoke Germano, merge FF-only, no false closure e Git come source of truth.
