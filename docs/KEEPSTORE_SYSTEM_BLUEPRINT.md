...
- KeepStore UI Typography Standard - ONSUS aligned: la fonte primaria e il template ONSUS gia presente nel repo, con stack `Inter`, body 15/24, title prodotto 22/25 peso 600, price principale 30/36, `price-text` 20/22, `body-md-2` 14/22, `body-text-3` 14/20, `caption` 12/22 e CTA `tf-btn` 15/24 peso 600. Colori fonte ONSUS: testo `#333e48`, muted `#73787d`, primary `#ff3d3d`, secondary/link `#004ec3`, border/panel `#ebebeb`/`#f5f5f5`. Le nuove pagine devono ereditare o riusare questo stack, mantenere line-height leggibile, gerarchia chiara, CTA touch-friendly su mobile e non introdurre font o CDN esterni.
- I task di singola pagina devono applicare typography solo in scope locale, evitando CSS globali aggressivi. La standardizzazione globale del sito e backlog riconciliato: prima `GLOBAL-TYPOGRAPHY-ONSUS-AUDIT-1` read-only, poi `GLOBAL-TYPOGRAPHY-ONSUS-1A/1B/...` con rollout progressivo. L'audit precede `ARTICOLO-CART-QTY-IN-PDP-AUDIT-1` e copre HOME, catalogo, PDP, carrello di riferimento, componenti condivisi e pagine pubbliche collegate con smoke desktop/mobile.
- KeepStore UI Typography Standard - ONSUS Computed Source of Truth: prima di dichiarare una pagina "allineata" va misurato il template ONSUS corrispondente con computed style reali, non solo leggendo CSS sorgente. Per la scheda prodotto PR #196 REV3 la fonte misurata e `Public/assets/keepstore/product-detail.html` con Chrome headless su desktop 1200, viewport 791 e mobile 390. Non inventare font-size, non usare `inherit` se il computed value resta diverso, applicare valori scoped solo dopo `GLOBAL-TYPOGRAPHY-ONSUS-AUDIT-1` e lasciare il rollout globale a `GLOBAL-TYPOGRAPHY-ONSUS-1A/1B/...`.
- Diagnosi typography da confermare nell'audit: `Page.master` carica `Public/assets/keepstore/fonts/font.css`; il file locale definisce MADE Outer e UTM Banque ma non risulta definire Inter. Le superfici che dichiarano `"Inter", serif` possono quindi risolvere un fallback serif indesiderato. `carrello.aspx`/`cart-ui.css` usa invece `"Inter", Arial, sans-serif`, base desktop `16px` con `line-height: 1.55` e mobile `15px`.
- `carrello.aspx` e il riferimento visuale approvato da Germano per famiglia percepita, fallback sans-serif, leggibilita, densita e resa desktop/mobile. La gerarchia dimensionale resta ONSUS/semantica: non uniformare tutto a `16px` o allo stesso peso/line-height; distinguere heading, titoli prodotto/sezione, body, prezzi, vecchi prezzi, metadata, caption, filtri, breadcrumb, CTA, badge, menu e card.
- Perimetro minimo `GLOBAL-TYPOGRAPHY-ONSUS-AUDIT-1`: `Default.aspx`/HOME (hero, settori, categorie, AI search, prodotti in evidenza, `Occasione Imperdibile`, renderer `Default.aspx.vb` e sezioni attive); `articoli.aspx` (toolbar, filtri/sidebar, applied filters, breadcrumb, ProductCard, titolo, metadata, prezzi, quantity/multiselect e CTA); `articolo.aspx` (titolo, breadcrumb, prezzo, disponibilita, metadata, descrizione, buy box, bundle, tab, Simili/Correlati/Recenti e CTA); `carrello.aspx`; `Page.master`, `SiteHeader.ascx`, `SiteFooter.ascx`, `MiniCart.ascx`, Quick View, Compare, `Breadcrumb.ascx` e controlli/pagine pubbliche che ereditano lo stack.
- Metodo audit typography: misurare computed styles e font realmente risolto su desktop/mobile, confrontare carrello/ONSUS/runtime, verificare disponibilita effettiva di Inter, CSP, overflow e touch target. Nessun CDN/font esterno, nuovo font o override globale aggressivo senza audit; `"Inter", Arial, sans-serif` e il riferimento iniziale, non una soluzione gia approvata.
...
- `GLOBAL-TYPOGRAPHY-ONSUS-AUDIT-1`: prossimo task read-only prioritario, prima di `GLOBAL-TYPOGRAPHY-ONSUS-1A/1B/...` e prima della quantita PDP; computed style/font risolto desktop-mobile su storefront pubblico, con `carrello.aspx` riferimento visuale e gerarchia ONSUS preservata.
