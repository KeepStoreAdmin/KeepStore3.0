...
- `COUPON-ESITO-PARAM-GUARD-1A` / PR #191 su branch `task/coupon-esito-param-guard-1a`, commit funzionale `5070568d88c22ae5d35a1df55fe2860533c76ae4`, corregge la robustezza di `coupon_esito_acquisto.aspx` quando il link viene aperto senza parametri validi. Causa individuata: `coupon_esito_acquisto.aspx.vb` concatenava `Request.QueryString("id")` e `Request.QueryString("cod")` nella `SelectCommand` di `SqlData_CouponInserzioni`, generando SQL malformato con `id` vuoto e `cod_controllo=''`. Fix: guard null-safe su `id`/`cod`, messaggio generico non tecnico, DataList scollegati dai DataSource su parametri invalidi e query valida parametrizzata. Asset `/Public/assets/images/headphone-2.svg` preservato; nessun cambio a DB/schema/SP, gateway/pagamenti, email/template, carrello/ordine/prezzi/totali o auth/sessioni/cookie. Prossimo step: review/merge PR #191 e smoke manuale pagina senza parametri e con link valido sicuro.
