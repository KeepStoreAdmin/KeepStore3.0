...
- Runtime stabile di base: `frontend-rebuild` / `origin/frontend-rebuild` a `f337de11e4a97588289b865caf7dfdccd5089c1c`.
...
- Ultimo task chiuso: `GOOGLE-PRODUCT-STRUCTURED-DATA-1A`, esito A, PR #263 integrata fast-forward nel checkpoint `f337de11e4a97588289b865caf7dfdccd5089c1c`; lo smoke Product Owner anonimo/PROVA ha confermato un solo Product/Offer, prezzo coerente e nessun dato personale o contaminazione cross-owner.
- Task corrente: `STOREFRONT-RUNTIME-ASSET-404-1A`, branch `task/storefront-runtime-asset-404-1a`, base stabile `f337de11e4a97588289b865caf7dfdccd5089c1c`. La REV2 certifica il modello reale di due applicazioni IIS separate con stessa release e database/catalogo condiviso; la REV3 rimuove il gradiente globale legacy estratto da `Coupon.master` e disattiva l'emissione automatica degli sfondi storici nel layout ONSUS. Host esatto e univoco continua a selezionare una sola riga `aziende`; la PR resta `DRAFT` fino al controllo visuale Product Owner.
- Prossimo task obbligatorio soltanto dopo chiusura A e merge: `MULTI-STOREFRONT-COMMERCIAL-ISOLATION-1A`, per certificare con test transazionali listini anonimo/registrazione/utente, autenticazione, carrelli, prezzi/promo, provenienza ordine e idempotenza storefront-scoped mantenendo la numerazione condivisa. Stato: `NON AVVIATO`. `GOOGLE-MERCHANT-CENTER-FEED-1A` resta sospeso fino alla sua chiusura.
