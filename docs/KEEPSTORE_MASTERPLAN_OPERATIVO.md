...
- `ACCOUNT-ADDRESS-ORDER-GUARD-1A` / PR #183 su branch `task/account-address-order-guard-1a`, commit funzionale `d7ca913aec57a5f5aebb1537ef08de07a71b3124`, aggiunge in `ordine.aspx.vb` una guard difensiva prima di `Carrello_Documento`: `Session("SCEGLIINDIRIZZO")` assente/null/0 mantiene il comportamento storico; valore non numerico/negativo o id > 0 non appartenente all'utente blocca l'ordine prima di SP, gateway, email e svuotamento carrello. La verifica ownership usa query parametrizzata su `utentiindirizzi.Id` + `UtenteId`; nessun cambio DB/schema/SP, carrello, gateway/pagamenti, email/template, prezzi/IVA/totali o dati cliente reali. Prossimo step: review/merge PR #183 e smoke controllato indirizzo valido/invalido senza gateway reale.
- `LOGIN-PASSWORD-TOGGLE-1A` / PR #184 su branch `task/login-password-toggle-1a`, commit funzionale `a1851c52dc1fb03af40a94618c068a5168504ff5`, corregge il doppio "Mostra password" su `login.aspx`: il toggle custom esistente resta l'unico controllo visibile e il reveal nativo browser viene nascosto in modo mirato sul campo password login. `login.aspx.vb`, logica auth, sessione/cookie, registrazione/reset/remind, DB/schema/SP, checkout/carrello/ordine/gateway ed email/template restano invariati. Prossimo step: review/merge PR #184 e smoke manuale login.
