...
Audit e contratto REV1 eseguiti sulla PR #268, base stabile `a4bb16fe354dc27226ac0e98e89d6bd438decce4`. Esito tecnico del contratto `A`, mentre runtime e rollout restano `NON AVVIATO`: la tabella `aziende` non riceve nuove colonne e conserva temporaneamente `Smtp`, `User_smtp`, `Password_smtp` ed `email`. Il modello legacy non rappresenta porta, TLS/auth espliciti, riferimento esterno alla credenziale, timeout, abilitazione o stato verifica; la password legacy resta da esternalizzare e ruotare.
...
Il contratto schema non applicato e versionato come `20260920_MULTIPROVIDER_TENANT_EMAIL_TRANSPORT_1A v2`: una sola tabella `aziende_email_transport` per database, PK tecnica `EmailTransportId`, FK a `aziende.id` e unique `(AziendeId, Purpose)`. Ogni azienda puo avere un profilo `TRANSACTIONAL` e, in futuro, uno `MARKETING`; tutti gli invii correnti migreranno sul primo. Host 253, identita e-mail/username 254, `CredentialReference` 512, display name Unicode e timeout 5-300 sono espliciti. I valori controllati sono testuali e validati, non `ENUM`, per evoluzione compatibile. Nessun profilo o segreto legacy viene copiato; default `Enabled=0`, attivazione solo dopo TLS/auth riusciti. Forward idempotente e rollback fail-closed con profili non sono autorizzati su database reali in questa fase.

MailKit e il candidato tecnico per un task successivo: supporta .NET Framework 4.8, STARTTLS esplicito, TLS implicito, SASL/XOAUTH2, timeout/cancellazione ed e distribuito con licenza MIT. Non e stato aggiunto: il sito non possiede oggi un manifest NuGet riproducibile, il pacchetto porta dipendenze transitive e OAuth2 richiede connettori, registrazione applicativa, consenso, refresh token e secret store non ancora disponibili. La compatibilita Google/Microsoft OAuth resta quindi `NON OPERATIVA`.

Il contratto operativo per Enzo e in `docs/MULTIPROVIDER_TENANT_EMAIL_CONFIG_ENZO.md`: una sola tabella per database, una riga per azienda/scopo, preset non autoritativi, credenziale mai riletta, verifica DNS/TCP/TLS/certificato/capability/AUTH terminata prima di `MAIL FROM`/`RCPT TO`/`DATA`, attivazione fail-closed e checklist per-database. Ordine dei gate, tutti `NON AVVIATO`: (1) `MULTIPROVIDER-TENANT-EMAIL-SCHEMA-ROLLOUT-1A`; (2) `SMTP-SECRET-EXTERNALIZATION-ROTATION-1A`; (3) pannello Enzo; (4) trasporto unico e migrazione chiamanti; (5) `SMTP-OAUTH-CONNECTORS-1A`; (6) retirement futuro dei campi legacy. PR #268 resta OPEN/DRAFT; nessuna migration o autenticazione live e autorizzata.
