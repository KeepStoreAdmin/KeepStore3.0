...
- Task corrente: `STOREFRONT-RUNTIME-ASSET-404-1A`, branch `task/storefront-runtime-asset-404-1a`, base stabile `f337de11e4a97588289b865caf7dfdccd5089c1c`. La REV2 certifica il modello reale di due applicazioni IIS separate con stessa release e database/catalogo condiviso: host esatto e univoco selezionano una sola riga `aziende`, sessione e asset vengono riallineati a quella riga, mentre host sconosciuti o ambigui falliscono chiusi. I riferimenti legacy CSS/sfondo inesistenti restano eliminati; test tecnici e HTTP sono positivi, ma la PR resta `DRAFT` fino al controllo visuale Product Owner.
...
- Causa storica sfondo: se la query `sfondi` non restituiva una riga attiva, il master costruiva automaticamente `Default{AziendaID}.png` sotto `Public/Sfondi/` e lo inseriva come `background-image` anche quando directory e file non esistevano.
- Regola permanente: CSS e sfondo tenant sono facoltativi. Il runtime accetta esclusivamente un filename locale, senza slash, schema, URL, `..`, caratteri di controllo o estensione fuori allowlist; verifica che il percorso fisico resti nella radice autorizzata e che il file esista. In caso contrario non emette il riferimento: restano attivi i CSS KeepStore/ONSUS standard e il colore/background grafico esistente.
