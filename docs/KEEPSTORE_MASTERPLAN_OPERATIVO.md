...
- `SESSION-POST-ORDER-LOGOUT-1A = B`: diagnosi statica mirata completata dopo segnalazione live "utente loggato dopo ordine completato viene rimandato a login cliccando Account dopo pochi minuti". `web.config` conferma sessione 30 minuti e nessun blocco FormsAuthentication applicativo; il sito usa Session come prova login (`LoginId`/`LoginID`). Nel percorso `ordine.aspx.vb` non risultano `Session.Clear`, `Session.Abandon`, `FormsAuthentication.SignOut` o rimozioni `LoginId/LoginID`; la pulizia post-ordine e selettiva su chiavi `Ordine_*`, coupon e token checkout. `Page.master` e header account riconoscono `LoginId` oppure `LoginID`; `myaccount.aspx` richiede `LoginId`. Causa non individuata con certezza e nessun fix runtime applicato.
- Ipotesi residua `SESSION-POST-ORDER-LOGOUT-1A`: perdita della Session ASP.NET InProc/session cookie o riciclo/app-domain da verificare live con confronto `SessionID`, presenza `LoginId/LoginID`, cookie sessione e timestamp prima/dopo ordine. Prossimo step consigliato: smoke diagnostico live controllato `SESSION-POST-ORDER-LOGOUT-1B` senza gateway reale e senza stampare cookie/secret.
