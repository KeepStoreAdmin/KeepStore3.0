...
Audit, contratto e migrazione completa dei chiamanti REV1 sono eseguiti sulla PR #268, base stabile `a4bb16fe354dc27226ac0e98e89d6bd438decce4`. Il 20 settembre 2026 la migration autorizzata e stata applicata a `taikun`: `aziende_email_transport` e presente e verificata. Il Product Owner ha inserito manualmente due profili `TRANSACTIONAL`, uno per ciascuna azienda prevista, entrambi `Enabled=0` e `VerificationStatus=NOT_VERIFIED`; i riferimenti correnti sono placeholder sanitizzati e intenzionalmente non operativi. Nessun file credenziale reale esiste e nessun profilo abilita invii. Credenziali reali transitate temporaneamente durante la preparazione sono state rimosse, considerate compromesse e destinate a rotazione coordinata senza recuperarne o documentarne i valori. Il backup pre-deployment resta fuori repository.
...
Con i due profili disabilitati, non verificati e privi di riferimento DPAPI, ogni flusso fallisce chiuso con esito sanitizzato senza contattare SMTP e senza ricadere sui campi legacy. La matrice sintetica copre 44 categorie di chiamanti e guardrail, 57 controlli runtime/facade e 43 controlli specifici del runner; restore dipendenze, precompile ASP.NET Framework 4.8, PowerShell, Markdown, diff e secret scan completano il gate senza e-mail, ordini o DML/DDL reali.

Il contratto operativo per Enzo resta `BOZZA TECNICA` in `docs/MULTIPROVIDER_TENANT_EMAIL_CONFIG_ENZO.md`: non autorizza pannello, profili o rollout. Il passaggio operativo successivo, ancora `NON AVVIATO`, e `MULTIPROVIDER-TENANT-EMAIL-CREDENTIAL-ROTATION-VERIFY-1A`: una casella alla volta, ruotare la credenziale compromessa presso il provider, generare sulla macchina destinataria il riferimento DPAPI, aggiornare manualmente il solo `CredentialReference`, eseguire il runner locale TLS/AUTH/QUIT, registrare manualmente l'esito e abilitare il profilo soltanto dopo `EMAIL_CONNECTION_VERIFIED`. DPAPI `LocalMachine` non e trasferibile: provisioning e verifica devono essere ripetuti sul server. OAuth2 e rimozione fisica dei campi legacy restano task futuri. `DATABASE-DUMP-REPOSITORY-HYGIENE-1A` resta separato.
