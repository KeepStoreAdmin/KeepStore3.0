...
- HEAD stabile: `792e7313e4a6abaae4c136e9fc1a9f915afcb755`
...
- `ORDER-NOTES-LIMIT-1A` registra anomalia live durante invio ordine: note checkout troppo lunghe causano errore tecnico DB `Data too long for column 'pNoteSpedizione' at row 1`. PR #173 su branch `task/order-notes-limit-1a`, commit iniziale `8d7fdc34293e6797cbe97751bf0d669ccf37e490`, aperta e tecnicamente revisionata; merge bloccato solo per integrazione documentale REV1. Fonte DB verificata: stored procedure `carrello_Documento`, parametro `pNoteSpedizione VARCHAR(255)`, salvato in `documenti.NoteEsterne` senza modifica schema. Fix implementato: limite UI 255 caratteri su `txtNoteSpedizione` in `carrello.aspx`, contatore/hint, validazione server-side in `carrello.aspx.vb` prima della creazione ordine e guard difensivo in `ordine.aspx.vb` prima di stored procedure, email e gateway. Se le note superano 255 caratteri non devono essere creati ordine, gateway o email e il carrello non deve essere svuotato; nessun troncamento silenzioso. Scope invariato: nessuna modifica DB/schema, gateway, prezzi, IVA, SMTP o template email. Smoke live ancora da eseguire dopo merge.
...
- Checkbox consenso condizioni: resta in PR #169 non mergiata e quindi non live su `frontend-rebuild`; non viene reimplementata in `ORDER-NOTES-LIMIT-1A`. Dopo il fix note va ripresa review/merge della PR #169.
- `CART-TERMS-CONSENT-1A-REV1`: la PR #169 storica su `task/cart-terms-consent-1a` non e recuperabile pulitamente dopo PR #173 per conflitti su `carrello.aspx` e `carrello.aspx.vb` e per asset CSS/JS fuori scope ammesso. Decisione operativa: branch sostitutivo `task/cart-terms-consent-1b` da `frontend-rebuild`, con consenso condizioni obbligatorio minimale in `carrello.aspx` / `carrello.aspx.vb`, pagina `condizioni-vendita.aspx` alimentata da `Aziende.Condizioni_vendita`, documentazione aggiornata e fix Note ordine PR #173 preservato integralmente. Regola: senza checkbox selezionata non devono partire creazione ordine, gateway o email e il carrello non deve essere svuotato; prossimo step review/merge e poi smoke manuale.
