...
- `ORDER-NOTES-LIMIT-1A` registra anomalia live durante invio ordine: note checkout troppo lunghe causano errore tecnico DB `Data too long for column 'pNoteSpedizione' at row 1`. Fonte DB verificata: stored procedure `carrello_Documento`, parametro `pNoteSpedizione VARCHAR(255)`, salvato in `documenti.NoteEsterne` senza modifica schema. Fix previsto: limite UI 255 caratteri su `carrello.aspx`, contatore/hint, validazione server-side in `carrello.aspx.vb` e guard difensivo in `ordine.aspx.vb` prima di stored procedure, email e gateway; nessun troncamento silenzioso, carrello preservato se la validazione fallisce.
