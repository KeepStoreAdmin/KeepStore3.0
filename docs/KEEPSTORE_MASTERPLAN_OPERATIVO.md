...
`SEO-DISCOVERY-CONFIG-SCHEMA-1A` progetta, ma non installa, `aziende_seo`: un'unica riga di configurazione SEO/discovery/provider per `AziendeId` (`UNIQUE`, 1:1 logico) con zero colonne nullable. Le credenziali appartengono alla stessa tabella solo come ciphertext in campi `Enc`, con `SecretsCryptoVersion` e `SecretsKeyId`; cifratura, gestione della master key esterna, reader runtime e provisioning restano task successivi. Stringa vuota significa comportamento automatico/fonte autorevole quando applicabile; `1000-01-01 00:00:00` significa scadenza non configurata o ignota e non autorizza l'uso di un token. Nessun flag di licenza puo disattivare le fondamenta SEO standard.

`aziende` resta source of truth di nome, `Descrizione` (Business Description), URL canonici/alias, logo, dati legali e commerciali. `aziende_seo` non copia Product, Offer, prezzi, promozioni, tassonomia o stato operativo IndexNow. I quattro mapping legacy sono `google_merchant_id -> GoogleMerchantId`, `facebook_pixel_id -> MetaPixelId`, `statistiche_visite -> TrackingScriptLegacy`, `facebookLink -> FacebookUrl`; la migration crea solo righe vuote, non trasferisce valori, non cancella colonne legacy e non cambia i reader. Il Product Owner effettuera la copia manuale soltanto in un rollout separato, seguito da parity e audit riferimenti prima di qualunque rimozione legacy. `TrackingScriptLegacy` e sola compatibilita: nessuna integrazione nuova deve immettervi HTML/JS arbitrario.

Il deployment dei quattro script canonici avverra manualmente in SQLyog sul solo database esplicitamente autorizzato dal Product Owner: ChatGPT preparera di volta in volta uno script operativo derivato dalla migration, evitando drift. Nessun nome database cliente e hardcoded nella migration; nessun SQL viene eseguito in questo task. Il futuro **Manuale Enzo "IKEA" - SEO & Discovery** descrivera menu, tab, label, controllo, campo, default, validazione, tooltip, mascheramento secret, salvataggio, test, screenshot e rimozione finale dei campi legacy; stato `FUTURE / NON AVVIATO`.

Addendum **Marketplace readiness**: `aziende_seo` predispone la configurazione tenant di TikTok Shop Seller (shop gia attivo per il Product Owner), Amazon SP-API ed eBay Sell/OAuth; nessun Shopify o middleware e prerequisito architetturale. Product/Offer restano autorevoli; listing, ordini marketplace, code, webhook, prezzi/stock inviati e telemetria restano nelle strutture operative esistenti o future, mai nella riga 1:1. I vecchi campi MWS Amazon e le configurazioni eBay legacy richiedono audit e migrazione separati, senza copia automatica. Prima di qualsiasi adapter TikTok Shop occorre audit dedicato Product/Order/Fulfillment/Return-Refund/Webhook. Il futuro Manuale Enzo includera il tab Marketplace (TikTok Shop, Amazon, eBay), con stato autorizzazione/scadenza e secret mascherati; nessuna UI o integrazione marketplace e implementata ora.
...
`COMMERCIAL-BULK-RESOLUTION-CONTEXT-1A` rimuove il prerequisito architetturale emerso nel preflight del reconciler: il resolver promo ora puo caricare un solo snapshot esplicito per database/azienda/listino/data/campagna/owner e riusarlo fuori da `HttpContext.Items`; il display promo e la disponibilita esplicita passano dagli stessi core usati dal web. Il contesto pubblico anonimo non legge Session e non crea un falso `HttpContext`. Non cambiano regole di prezzo, promo o disponibilita, ne i call-site WebForms. `INDEXNOW-PUBLIC-STATE-RECONCILER-1A` resta il prossimo task, non avviato da questa modifica; dovra consumare questi resolver, senza motore commerciale parallelo.
...
#### Sequenza runtime immediata

La documentazione autorizza come prossima sequenza, ciascun task separato e soggetto ai normali gate A/B/E:
1. `HOME-TENANT-IDENTITY-SEO-1A`: usare identita/Business Description tenant, eliminare claim e social fallback tech non autorevoli.
2. `HOME-MULTIMERCH-FALLBACK-DEHARDEN-1A`: rimuovere i fallback elettronica residui dalla HOME e fallire soft/nascondere quando mancano dati reali.
3. `SEO-2026-CENTRALIZATION-1A`: audit e convergenza progressiva di metadata/structured-data helpers verso un contratto comune, senza riscrivere canonical/robots/sitemap/Product-Offer gia sani.

Nessuno dei tre autorizza DDL, nuovo motore promo, vector DB, embeddings, RAG, Merchant feed o deploy automatico.
