...
- Runtime stabile di base: `frontend-rebuild` / `origin/frontend-rebuild` a `07522fa332c98b83bd535abd090d0d8899ee262a`.
...
- Ultimo task chiuso: `KEEPSTORE-MULTITENANT-ONBOARDING-CONTRACT-1A`, PR #262 integrata fast-forward nel checkpoint `07522fa332c98b83bd535abd090d0d8899ee262a`; il contratto vieta personalizzazioni cliente nel runtime condiviso e rende obbligatori dati, asset e host tenant autorevoli.
- Task corrente: `GOOGLE-PRODUCT-STRUCTURED-DATA-1A`, branch `task/google-product-structured-data-1a`, base stabile `07522fa332c98b83bd535abd090d0d8899ee262a`. Il contratto e tecnicamente implementato sulla sola PDP; la verifica HTTP anonima e i test isolati sono positivi, mentre il gate HTTP autenticato PROVA resta da completare con una sessione sicura. Fino ad allora l'esito complessivo e `E` e la PR resta `DRAFT`.
- Prossimo task consigliato soltanto dopo review A e merge: `GOOGLE-MERCHANT-CENTER-FEED-1A`, per progettare il feed multi-tenant usando lo stesso contratto commerciale autorevole, senza duplicare resolver o prezzi. Stato: `NON AVVIATO`.
...
- Copertura tecnica: 18 gruppi fixture / 46 asserzioni, due tenant sintetici e host sconosciuto, prezzo normale/promo/tier, disponibilita, GTIN, immagini, caratteri speciali, anonimo e contesto owner-scoped simulato; query DB aggiuntive del builder `0`. Runtime anonimo verificato con un solo `Product` e un solo `Offer`, prezzo visibile coerente e nessun dato personale. Il gate HTTP autenticato PROVA resta necessario prima dell'esito A.
