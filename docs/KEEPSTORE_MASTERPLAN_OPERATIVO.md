...
- Task corrente: `STOREFRONT-RUNTIME-ASSET-404-1A`, branch `task/storefront-runtime-asset-404-1a`, base stabile `f337de11e4a97588289b865caf7dfdccd5089c1c`. Elimina esclusivamente i riferimenti tenant legacy a CSS e sfondo inesistenti; test tecnici e HTTP sono positivi, mentre il controllo visuale desktop/mobile resta al Product Owner perche il browser interattivo non e disponibile. La PR resta `DRAFT` fino a quel gate.
...
- Prove: 10 fixture multi-tenant / 16 asserzioni sul resolver; HOME, catalogo, PDP, carrello e login HTTP 200; 83 asset first-party verificati e zero non-200; i due riferimenti legacy assenti; Product JSON-LD, SEO e promo invariati. Residuo: controllo visuale Product Owner su PDP desktop, `390x844` e `360x800` per confermare assenza di flash, sfondo rotto o perdita stili.
