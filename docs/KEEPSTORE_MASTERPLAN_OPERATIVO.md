...
Stato di riferimento corrente dopo chiusura scheda articolo / disponibilita / promo:

- Branch stabile: `frontend-rebuild`
- HEAD stabile: `eaf2618a7a243a43fc733f906061f28a52a559cc`
...
- Chiusura blocco scheda articolo: PR #196, PR #197 e PR #198 sono mergeate su `frontend-rebuild`; HEAD stabile post-merge PR #198 `eaf2618a7a243a43fc733f906061f28a52a559cc`, `main` invariato `976e99f17cabc8a5c6a8715463444edfeaadcd91`. La scheda `articolo.aspx` e ora riferimento per layout ONSUS, disponibilita `Aziende.DispoTipo`, promo/offerte legacy modernizzate, tooltip `In Arrivo` e contesto Webaffare listino 10. Test ufficiale: `articolo.aspx?id=12384`, variante `articolo.aspx?id=12384&TCid=-1`, Host simulato `www.webaffare.it` e confronto legacy `https://www.webaffare.it/articolo.aspx?id=12384&TCid=-1`. Regola campo: usare sempre `DispoTipo` (`1` sintetico/pallini, `2` numerico), mai `DispTipo`. Smoke finale PR #198: con `DispoTipo=2` validati `Disponibilita: 9`, `Impegnati: 3`, `In Arrivo: 2`, stato `Disponibile` verde; `9` e corretto per DB live aggiornato dopo fatturazione. Tooltip `In Arrivo`: riga compatta, icona info CSS scoped senza asset immagine, testo lungo non fisso, fallback `title`/`aria-label`/`data-tooltip`. Promo Webaffare/listino 10: prezzo listino `40,00`, prezzo standard `40,00`/`36,00` dove previsto, prezzo finale `9,76`, sconto `76%`, `MULTIPLI 10`, `MINIMO 1`, `MULTIPLI 5`, date `25/06/2026-31/07/2026`, promo scadute escluse da filtro data. Multi-azienda: stesso DB per piu siti, filtro `AziendeId` + listino + data, Webaffare su listino 10 e prezzi IVA inclusa; le promo non devono attraversare aziende/listini. Home, catalogo, carrello, `cart_add.aspx` e ordine non sono stati modificati da PR #198. `Public/assets/images/articoli/` resta asset directory non tracciata fuori scope.
- Backlog separato obbligatorio: `CART-PROMO-PRICE-REVALIDATION-1A` dovra auditare/implementare la revalidazione prezzi promo in carrello/ordine prima della conferma finale, cosi se una promo scade o cambia dopo l'inserimento nel carrello il cliente riceve un avviso e riconferma consapevolmente i nuovi prezzi. Punto di partenza audit: `aggiungi.aspx.vb`/`cart_add.aspx.vb` per inserimento, `carrello.aspx.vb` per ricalcolo e step checkout, `ordine.aspx.vb` prima di `Carrello_Documento`. Non e parte della PR #198.
- Backlog successivo non immediato: estendere `DispoTipo` e promo/offerte a catalogo/product card, poi Home e carrello con micro-task separati; eventuale ottimizzazione grafica generale resta futura e non riapre la chiusura PR #198.
