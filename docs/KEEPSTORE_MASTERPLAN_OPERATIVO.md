...
- Checkbox consenso condizioni: PR #169 storica non mergiata e sostituita da PR #174; il consenso non e ancora live su `frontend-rebuild` finche PR #174 non viene mergiata. Non viene reimplementata in `ORDER-NOTES-LIMIT-1A`.
- `CART-TERMS-CONSENT-1B` / PR #174 su branch `task/cart-terms-consent-1b`, commit funzionale `3b20195aca524c06cd13e7f4a88cac84af96dc8e`: sostituisce PR #169 non recuperata per conflitti con PR #173 su `carrello.aspx` / `carrello.aspx.vb` e per asset CSS/JS fuori scope. Stato REV1: PR #174 open/non draft/mergeable e tecnicamente revisionata; merge bloccato solo per integrazione documentale. Regola: consenso condizioni vendita obbligatorio prima di creare ordine, gateway o email; preservato fix PR #173 limite note ordine 255, contatore/hint, validazione server-side e guard difensiva. Prossimo step: rilanciare `CART-TERMS-CONSENT-1C` review/merge, poi smoke manuale consenso condizioni e non regressione limite note ordine.
