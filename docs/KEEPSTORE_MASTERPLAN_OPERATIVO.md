...
- `HOME-DEPARTMENTS-MENU-MOBILE-AUDIT-1A` ha rilevato due finding distinti: le route `st`/`st+ct` venivano ristrette a un primo livello figlio, mentre collapse ARIA, indicatori e target touch del menu mobile restano incoerenti. `CATALOG-TAXONOMY-ROUTE-SEMANTICS-1A` corregge solo il primo finding sul branch task: settore = `st`, categoria = `st+ct`, tipologia = `st+ct+tp` soltanto se richiesta esplicitamente; una categoria senza settore o con settore incoerente determina/corregge `st` autorevole senza inventare `ct` o `tp`. Smoke locale GET: settore, categoria e tipologia mantengono URL distinti; `ct=35` mostra le tre tipologie reali invece di limitarsi alla prima; canonical coerenti, precompile PASS. Il finding mobile ARIA/`+/-`/touch resta aperto per `MOBILE-CATALOG-COLLAPSE-A11Y-1A` (NON AVVIATO). Nessun DB write, modifica menu o merge in questo task.
