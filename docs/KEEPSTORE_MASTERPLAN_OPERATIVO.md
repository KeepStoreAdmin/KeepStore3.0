...
Audit eseguito sulla PR #268, base stabile `a4bb16fe354dc27226ac0e98e89d6bd438decce4`. Esito corrente `B`: il runtime non puo ancora essere reso multi-provider senza prima introdurre un contratto configurativo tenant-scoped e autorizzare la relativa migration. La tabella `aziende` espone soltanto `Smtp`, `User_smtp`, `Password_smtp` ed `email`; non rappresenta porta, modalita TLS, modalita di autenticazione, riferimento esterno alla credenziale, timeout, abilitazione o stato di verifica. La password legacy resta in chiaro e viene anche copiata in Session da `Page.master`.
...
La proposta schema non applicata e versionata come `20260920_MULTIPROVIDER_TENANT_EMAIL_TRANSPORT_1A`: tabella `aziende_email_transport` owner-scoped da `AziendeId`, profilo inizialmente disabilitato, modalita `StartTls`/`ImplicitTls`, autenticazione `Password`/`AppPassword`/`OAuth2`, mittenti, timeout, stato verifica e solo `CredentialReference`. Non copia `Password_smtp`, token o altri segreti. Forward idempotente e rollback protetto non sono autorizzati all'esecuzione in questa fase.
...
Ordine dei gate successivi, tutti `NON AVVIATO`: (1) autorizzazione e rollout allowlist della migration proposta; (2) `SMTP-SECRET-EXTERNALIZATION-ROTATION-1A`; (3) `SMTP-OAUTH-CONNECTORS-1A`; (4) implementazione del trasporto unico e migrazione di tutti i chiamanti; (5) pannello tenant, test connessione e onboarding SPF/DKIM/DMARC; (6) rotazione di entrambe le credenziali legacy soltanto dopo trasporto cifrato verificato. PR #268 resta OPEN/DRAFT e nessun invio live e autorizzato.
