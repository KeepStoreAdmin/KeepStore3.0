...
- `EMAIL-ORDER-CONFIRMATION-PRODUCT-IMAGES-1C = E`: smoke live bloccato per email non ricevuta; ordine test bonifico creato e gateway reale non avviato, ma immagini prodotto non valutabili. Prima di ulteriori smoke ordine/email, nuova priorita `CART-TERMS-CONSENT-1A`.
- `CART-TERMS-CONSENT-1A` introduce nel riepilogo finale carrello una checkbox obbligatoria non preselezionata per accettare Condizioni Generali di Vendita, diritto di recesso e Privacy Policy prima del pulsante finale. Il blocco e validato lato client/UX e lato server: senza consenso non devono partire creazione ordine, gateway o email; con consenso il flusso resta invariato. Il pulsante finale diventa `INVIA ORDINE CON OBBLIGO DI PAGAMENTO`.
- `CART-TERMS-CONSENT-1A = B`: prima verifica PR #169 non accettabile per fallback condizioni/recesso su `Contattaci.aspx`, test manuale non ancora completato e segnalazione live di logout post-ordine dopo pochi minuti.
- `CART-TERMS-CONSENT-1A-REV1` corregge i link legali del consenso: Condizioni Generali di Vendita e diritto di recesso non puntano piu a `Contattaci.aspx`, ma a `condizioni-vendita.aspx` / `condizioni-vendita.aspx#diritto-recesso`, pagina read-only alimentata da `Aziende.Condizioni_vendita` con query parametrizzata e rendering filtrato. La Privacy Policy resta su `privacy.aspx`.
- Nota contenuti legali: il diritto di recesso e considerato incluso nella pagina condizioni; il contenuto specifico deve essere presente e mantenuto nel gestionale dentro `Aziende.Condizioni_vendita`.
- Audit logout post-ordine `CART-TERMS-CONSENT-1A-REV1`: nel percorso `ordine.aspx.vb` non risultano `Session.Clear`, `Session.Abandon`, `FormsAuthentication.SignOut` o rimozione di `LoginId`; vengono azzerate solo chiavi ordine/coupon e consumato il token checkout. Nessun fix runtime e stato applicato perche la causa della segnalazione live non e chiara.
- Backlog sessione: `SESSION-POST-ORDER-LOGOUT-1A` per diagnosi mirata su ambiente live/log quando l'utente risulta non loggato dopo ordine, senza toccare gateway, documenti, pagamento, email o sessione finche la causa non e riproducibile.
- Backlog legale/futuro: `CART-TERMS-CONSENT-AUDIT-1A` per eventuale salvataggio data/IP/versione condizioni, solo con futura modifica DB autorizzata; in `CART-TERMS-CONSENT-1A` nessun DB/schema/SQL viene modificato.
