...
Stato Git stabile dopo la chiusura PDP CTA `Acquista`:

- `frontend-rebuild` e `origin/frontend-rebuild`: `3b0b2ac97564c497abd26d224e5e945834a2ec26`.
...
- PDP `articolo.aspx`: chiusi esclusivamente `PDP-BUY-CTA-ACQUISTA-AUDIT-1` e runtime `PDP-BUY-CTA-ACQUISTA-1A/1B`; la pagina non e dichiarata completa. Typography globale, quantita gia nel carrello PDP, side cart e altre evoluzioni restano task separati.
...
1. Priorita 1 - PDP / scheda prodotto `articolo.aspx`.
   ...
   - `GLOBAL-TYPOGRAPHY-ONSUS-AUDIT-1`: PROSSIMO TASK, audit read-only globale.
   - `GLOBAL-TYPOGRAPHY-ONSUS-1A/1B/...`: DA FARE DOPO AUDIT, rollout scoped e progressivo.
   - `ARTICOLO-CART-QTY-IN-PDP-AUDIT-1`: DA FARE DOPO TYPOGRAPHY.
   - `ARTICOLO-CART-QTY-IN-PDP-1A`: runtime solo dopo audit.
...
- `GLOBAL-TYPOGRAPHY-ONSUS-AUDIT-1` precede qualsiasi implementazione typography e il successivo audit quantita PDP. L'audit deve misurare computed style reali desktop/mobile e font effettivamente risolti, confrontando runtime, ONSUS reale e `carrello.aspx`.
- Perimetro minimo: `Default.aspx`/HOME e card generate da `Default.aspx.vb` (hero, menu settori, categorie, AI search, prodotti in evidenza, `Occasione Imperdibile` e sezioni attive), `articoli.aspx` (toolbar, filtri/sidebar, applied filters, breadcrumb, ProductCard, titolo, metadata, prezzi, quantita/multiselect e CTA), `articolo.aspx` (titolo, breadcrumb, prezzo, disponibilita, metadata, descrizione, buy box, bundle, tab, Simili/Correlati/Recenti e CTA) e `carrello.aspx` come riferimento visuale.
- Componenti condivisi inclusi: `Page.master`, `SiteHeader.ascx`, `SiteFooter.ascx`, `MiniCart.ascx`, Quick View, Compare, `Breadcrumb.ascx` e altri controlli storefront collegati; le altre pagine pubbliche che ereditano lo stesso stack vanno prima mappate, non modificate automaticamente.
- Diagnosi da verificare: `Page.master` carica `Public/assets/keepstore/fonts/font.css`, che oggi definisce MADE Outer e UTM Banque ma non risulta definire localmente Inter; regole `"Inter", serif` possono quindi ricadere su un serif indesiderato. `carrello.aspx`/`cart-ui.css` usa `"Inter", Arial, sans-serif`, base desktop `16px` con `line-height: 1.55` e mobile `15px`.
- `carrello.aspx` e il riferimento visuale approvato da Germano per famiglia percepita, fallback sans-serif, leggibilita, densita e resa desktop/mobile. Non significa copiare `16px` ovunque: ONSUS resta fonte per la gerarchia tra heading, titoli prodotto/sezione, body, prezzi, vecchi prezzi, metadata, caption, filtri, breadcrumb, CTA, badge, menu e card.
- Regole audit: niente affidamento sul solo CSS sorgente, CDN/font esterni o nuovi font senza audit, rispetto CSP, niente override globali aggressivi; strategia iniziale `"Inter", Arial, sans-serif` da confermare, rollout scoped progressivo e verifica overflow/touch target.
...
Stato di riferimento corrente dopo chiusura e merge fast-forward `PDP-BUY-CTA-ACQUISTA-1A/1B`:

- Branch stabile: `frontend-rebuild`
- HEAD stabile locale/origin: `3b0b2ac97564c497abd26d224e5e945834a2ec26`
...
- KeepStore UI Typography Standard - ONSUS aligned: ogni nuova pagina o refactoring deve partire dal template ONSUS gia integrato nel repo, ereditare o riusare lo stack `Inter`, evitare font-family inventate e CSS globali aggressivi, mantenere scala coerente per title/subtitle/body/small/price/buttons, line-height leggibile, CTA touch-friendly su mobile, testo non miniaturizzato sotto 12px salvo label tecniche, contrasto e gerarchia chiari e righe testuali non eccessivamente lunghe. La standardizzazione globale segue prima `GLOBAL-TYPOGRAPHY-ONSUS-AUDIT-1` read-only e poi `GLOBAL-TYPOGRAPHY-ONSUS-1A/1B/...`, con computed-style audit, design token tipografici e applicazione progressiva a HOME, catalogo, scheda prodotto, account e carrello senza regressioni visuali.
