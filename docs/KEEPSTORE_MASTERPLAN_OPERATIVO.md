...
Audit e contratto REV1 eseguiti sulla PR #268, base stabile `a4bb16fe354dc27226ac0e98e89d6bd438decce4`. Il 20 settembre 2026 la migration autorizzata e stata applicata a `taikun`: `aziende_email_transport` e presente e verificata. Il Product Owner ha inserito manualmente due profili `TRANSACTIONAL`, uno per ciascuna azienda prevista, entrambi `Enabled=0` e `VerificationStatus=NOT_VERIFIED`; i riferimenti correnti sono placeholder sanitizzati, nessun file credenziale reale esiste e nessun profilo e operativo. Credenziali reali transitate temporaneamente durante la preparazione sono state rimosse dalle righe correnti, sono considerate compromesse e devono essere ruotate senza recuperarne o documentarne i valori. Il backup pre-deployment resta fuori repository. La tabella `aziende` non riceve nuove colonne e conserva temporaneamente `Smtp`, `User_smtp`, `Password_smtp` ed `email`; la password legacy resta da esternalizzare e ruotare.

Il trasporto e inoltre frammentato: ordine, contatti, registrazione/profilo e reset password non condividono ancora un unico loader/transport. `System.Net.Mail.SmtpClient` non copre TLS implicito e OAuth2 e, nei percorsi correnti, non configura TLS in modo esplicito. Nessun provider puo essere dedotto da dominio e-mail o porta; nessun preset puo prevalere sui valori autorevoli della singola azienda.
...
Il nucleo provider-neutral e implementato. MailKit `4.18.0` e MimeKit `4.18.0`, con dipendenze transitive fissate, licenze, SHA-256 e restore dalla sorgente NuGet ufficiale, sono distribuiti nelle DLL `Bin` richieste dal Web Site .NET Framework 4.8. Resolver e trasporto sono tenant-scoped per identita database, azienda e scopo; STARTTLS e TLS implicito sono espliciti, certificato e hostname obbligatori, plaintext/downgrade vietati, log sanitizzati e stati tecnici fail-closed. Il secret store usa DPAPI `LocalMachine` fuori dal webroot e riferimenti opachi. Il solo tool amministrativo `Tools/Set-EmailTransportCredential.ps1` deriva l'identita database dal `web.config`, richiede due volte la credenziale con input mascherato, configura ACL protette per Administrators, SYSTEM e identita effettiva dell'Application Pool e scrive atomicamente; non accetta password o database identity da riga di comando, non scrive il database e non crea endpoint, listener o modifiche IIS. Il precedente harness UAC con sonda IIS e escluso dal contratto e dal commit. OAuth2 Google/Microsoft resta `NON OPERATIVO` fino a `SMTP-OAUTH-CONNECTORS-1A`.

Il contratto operativo per Enzo resta `BOZZA TECNICA` in `docs/MULTIPROVIDER_TENANT_EMAIL_CONFIG_ENZO.md`: non autorizza pannello, profili o rollout. Invio e verifica amministrativa sono separati: l'invio richiede `Enabled=1`, stato `VERIFIED` e credenziale risolvibile; la verifica puo caricare esclusivamente un profilo completo `NOT_VERIFIED`/`VERIFIED` anche disabilitato, ma richiede un vero riferimento DPAPI, si limita a TLS/AUTH/QUIT e non modifica automaticamente database o stato. I placeholder restano non operativi. Nessun chiamante legacy usa ancora il nuovo trasporto. Prossimo task `MULTIPROVIDER-TENANT-EMAIL-CREDENTIAL-ROTATION-VERIFY-1A`, `NON AVVIATO`: rotazione coordinata, generazione locale dei due riferimenti, aggiornamento manuale delle righe e verifica di connessione senza invio; solo dopo potra essere autorizzata la migrazione separata dei chiamanti. OAuth2 e retirement dei campi legacy restano task futuri. `DATABASE-DUMP-REPOSITORY-HYGIENE-1A` resta `NON AVVIATO` e separato.
