...
- `AVAILABILITY-DISPTIPO-CORE-1A` apre il blocco disponibilita numerica con helper centrale `App_Code/AvailabilityDisplayHelper.vb` basato su `Aziende.DispoTipo` gia in `Session("DispoTipo")`: `1` mantiene la disponibilita sintetica/pallini, `2` abilita righe numeriche `Disponibilita`, `Impegnati`, `In Arrivo` e stato verde/rosso. Applicazione iniziale limitata a `articolo.aspx.vb`; home, catalogo, product card e carrello restano fuori scope per task dedicati. Nessun cambio a DB/schema/SP, prezzi, promo, carrello, ordine, gateway, email/template, auth/sessioni/cookie o PR #196.
