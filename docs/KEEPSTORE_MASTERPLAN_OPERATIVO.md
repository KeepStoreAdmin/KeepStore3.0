...
- Task corrente: `MULTI-STOREFRONT-ORDER-PROVENANCE-EMAIL-1A`, branch `task/multistorefront-order-provenance-email-1a`, base stabile `2d6ebd540a29ff906a01fa0820ebf65706150408`. La PR #267 resta DRAFT: provenienza, replay, documenti ed e-mail sono tenant-aware, mentre la REV2 corregge il submit checkout silenzioso e attende lo smoke conclusivo del Product Owner.
- Prossimo task soltanto dopo review A, smoke autorizzato e merge della PR #267: `STOREFRONT-PERSISTENT-ANONYMOUS-CART-1A`, stato `NON AVVIATO`. Seguono `STOREFRONT-CART-CONVERSION-UX-1A`, `GOOGLE-MERCHANT-CENTER-FEED-1A` e, solo dopo il contratto privacy/consenso, `STOREFRONT-ABANDONED-CART-RECOVERY-1A`; nessuno e avviato.
...
- Roadmap obbligatoria dopo la chiusura della PR #267: (1) `STOREFRONT-PERSISTENT-ANONYMOUS-CART-1A`, 30 giorni dall'ultima attivita, token casuale opaco, cookie host-only `Secure`/`HttpOnly`/`SameSite=Lax`, isolamento database + `AziendaId`, righe DB, merge login idempotente una sola volta, rotazione e invalidazione, revalidation prezzi/promo/stock, nessuna prenotazione inventario, cleanup e aggiornamento privacy/cookie policy; (2) `STOREFRONT-CART-CONVERSION-UX-1A`, inclusi stato `carrello salvato`, soglia spedizione e variazioni prezzo/stock; (3) `GOOGLE-MERCHANT-CENTER-FEED-1A`; (4) `STOREFRONT-ABANDONED-CART-RECOVERY-1A` soltanto per utenti autenticati e dopo contratto privacy/consenso, senza spam. Tutti sono `NON AVVIATO`.
