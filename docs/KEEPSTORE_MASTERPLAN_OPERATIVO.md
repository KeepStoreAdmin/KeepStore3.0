...
Con i due profili disabilitati, non verificati e privi di riferimento DPAPI, ogni flusso fallisce chiuso con esito sanitizzato senza contattare SMTP e senza ricadere sui campi legacy. La matrice sintetica copre 44 categorie di chiamanti e guardrail, oltre a 56 controlli runtime/facade; restore dipendenze, precompile ASP.NET Framework 4.8, PowerShell, Markdown, diff e secret scan completano il gate senza e-mail, ordini o DML/DDL reali.

Il contratto operativo per Enzo resta `BOZZA TECNICA` in `docs/MULTIPROVIDER_TENANT_EMAIL_CONFIG_ENZO.md`: non autorizza pannello, profili o rollout. L'unico passaggio operativo residuo, separato e `NON AVVIATO`, e `MULTIPROVIDER-TENANT-EMAIL-CREDENTIAL-ROTATION-VERIFY-1A`: ruotare le credenziali compromesse presso i provider, generare localmente i due riferimenti DPAPI, aggiornare manualmente `CredentialReference`, verificare TLS/AUTH/QUIT, registrare `VERIFIED` e abilitare i profili. OAuth2 e rimozione fisica dei campi legacy restano task futuri. `DATABASE-DUMP-REPOSITORY-HYGIENE-1A` resta separato.
