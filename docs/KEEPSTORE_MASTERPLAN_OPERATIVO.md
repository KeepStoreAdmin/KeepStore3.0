...
- Task corrente: `STOREFRONT-RUNTIME-ASSET-404-1A`, branch `task/storefront-runtime-asset-404-1a`, base stabile `f337de11e4a97588289b865caf7dfdccd5089c1c`. La REV1 rende esplicito il modello same-database/multi-storefront: host esatto e univoco selezionano una sola riga `aziende`, sessione e asset vengono riallineati a quella riga, mentre host sconosciuti o ambigui falliscono chiusi. I riferimenti legacy CSS/sfondo inesistenti restano eliminati; test tecnici e HTTP sono positivi, ma la PR resta `DRAFT` fino al controllo visuale Product Owner.
- Prossimo task consigliato soltanto dopo chiusura A e merge: `GOOGLE-MERCHANT-CENTER-FEED-1A`, per progettare il feed multi-tenant usando lo stesso contratto commerciale autorevole, senza duplicare resolver o prezzi. Stato: `NON AVVIATO`.
...
- Prove: 10 fixture / 16 asserzioni asset e 33 asserzioni same-database su due vetrine sintetiche con catalogo condiviso, canonical/alias distinti, CSS diversi, sequenza A-B-A, host sconosciuto e duplicato; HOME, catalogo, PDP, carrello e login HTTP 200 sull'host tenant valido e riferimenti legacy assenti. Product JSON-LD, SEO e promo restano invariati. Residuo: controllo visuale Product Owner su PDP desktop, `390x844` e `360x800` per confermare assenza di flash, sfondo rotto o perdita stili.
