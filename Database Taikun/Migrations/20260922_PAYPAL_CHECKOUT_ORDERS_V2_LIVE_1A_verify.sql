...
SELECT 'TRANSACTION_UNIQUES' AS Controllo, CASE WHEN COUNT(DISTINCT INDEX_NAME)=5 THEN 'OK' ELSE 'STOP' END AS Esito
FROM information_schema.STATISTICS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='paypal_checkout_transazioni'
AND INDEX_NAME IN ('UX_paypal_checkout_tx_document','UX_paypal_checkout_tx_order','UX_paypal_checkout_tx_capture','UX_paypal_checkout_tx_create_request','UX_paypal_checkout_tx_capture_request');
