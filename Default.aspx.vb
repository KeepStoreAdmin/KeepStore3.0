...
Partial Public Class _Default
    ...
    Private Sub BindHome()
        ...
        Dim offerPool As DataTable = ShuffleTable(GetOfferPool(120))
        ...
        Dim dealPool As DataTable = TakeDistinctRows(12, usedIds, offerPool, bestSellerPool, mostViewedPool)
        rptDealOfDay.DataSource = dealPool
        rptDealOfDay.DataBind()

        BindThreeColumnZone(TakeDistinctRows(7, usedIds, offerPool, bestSellerPool), rptFeatureLeft, rptFeatureCenter, rptFeatureRight)
        BindThreeColumnZone(TakeDistinctRows(7, usedIds, mostViewedPool, featuredPool), rptToprateLeft, rptToprateCenter, rptToprateRight)
        BindThreeColumnZone(TakeDistinctRows(7, usedIds, newArrivalsPool, offerPool), rptOnSaleLeft, rptOnSaleCenter, rptOnSaleRight)

        rptBestSeller.DataSource = TakeDistinctRows(12, usedIds, bestSellerPool, topSellingPool, offerPool)
        rptBestSeller.DataBind()

        rptTop20Slides.DataSource = BuildSlidesTable(TakeDistinctRows(10, usedIds, offerPool, bestSellerPool), 5)
        rptTop20Slides.DataBind()

        rptFeaturedProductsSlides.DataSource = BuildSlidesTable(TakeDistinctRows(10, usedIds, featuredPool, mostViewedPool), 5)
        rptFeaturedProductsSlides.DataBind()

        rptTopSellingProductSlides.DataSource = BuildSlidesTable(TakeDistinctRows(10, usedIds, topSellingPool, bestSellerPool), 5)
        rptTopSellingProductSlides.DataBind()

        rptOnSaleProductSlides.DataSource = BuildSlidesTable(TakeDistinctRows(10, usedIds, bestSellerPool, offerPool), 5)
    ...
    Private Sub BindThreeColumnZone(ByVal source As DataTable, ByVal leftRepeater As Repeater, ByVal centerRepeater As Repeater, ByVal rightRepeater As Repeater)
        Dim work As DataTable = EnsureMinimumRows(source, 7)
    ...
    Private Function BuildSlidesTable(ByVal source As DataTable, ByVal groupSize As Integer) As DataTable
        ...
        If work Is Nothing OrElse work.Rows.Count = 0 Then
            work = SampleProducts(Math.Max(groupSize * 2, 10))
        End If
    ...
    Private Function GetOfferPool(ByVal limit As Integer) As DataTable
        Return QueryProducts("COALESCE(v.InOfferta,0)=1 AND COALESCE(v.PrezzoPromoIvato,0)>0 AND COALESCE(v.Giacenza,0)>=1", "COALESCE(v.Visite,0) DESC, COALESCE(v.DataCreazione,CURDATE()) DESC, v.id DESC", limit)
    End Function

    Private Function GetFeaturedPool(ByVal limit As Integer) As DataTable
        Return QueryProducts("COALESCE(v.Vetrina,0)=1 AND COALESCE(v.Giacenza,0)>=1", "COALESCE(v.Vetrina,0) DESC, COALESCE(v.Visite,0) DESC, COALESCE(v.DataCreazione,CURDATE()) DESC, v.id DESC", limit)
    End Function

    Private Function GetMostViewedPool(ByVal limit As Integer) As DataTable
        Return QueryProducts("COALESCE(v.Giacenza,0)>=1", "COALESCE(v.Visite,0) DESC, COALESCE(v.DataCreazione,CURDATE()) DESC, v.id DESC", limit)
    End Function

    Private Function GetNewArrivalsPool(ByVal limit As Integer) As DataTable
        Return QueryProducts("COALESCE(v.Giacenza,0)>=1", "COALESCE(v.DataCreazione,CURDATE()) DESC, COALESCE(v.Visite,0) DESC, v.id DESC", limit)
    End Function

    Private Function GetBestSellerPool(ByVal limit As Integer) As DataTable
        Return QueryProducts("COALESCE(v.InOfferta,0)=1 AND COALESCE(v.PrezzoPromoIvato,0)>0 AND COALESCE(v.Giacenza,0)>=1", "COALESCE(s.QtaVenduta,0) DESC, COALESCE(v.Visite,0) DESC, COALESCE(v.DataCreazione,CURDATE()) DESC, v.id DESC", limit)
    End Function

    Private Function GetPureTopSellingPool(ByVal limit As Integer) As DataTable
        Return QueryProducts("COALESCE(v.Giacenza,0)>=1", "COALESCE(s.QtaVenduta,0) DESC, COALESCE(v.Visite,0) DESC, COALESCE(v.DataCreazione,CURDATE()) DESC, v.id DESC", limit)
    End Function
    ...
    Private Function QueryProducts(ByVal whereClause As String, ByVal orderClause As String, ByVal limit As Integer) As DataTable
        ...
        sql.Append("COALESCE(v.Stato,0) AS Stato, COALESCE(v.Giacenza,0) AS Giacenza, COALESCE(v.Disponibilita,0) AS Disponibilita, COALESCE(v.Impegnata,0) AS Impegnata, ")
        sql.Append("COALESCE(v.PrezzoIvato,0) AS PrezzoIvato, COALESCE(v.PrezzoPromoIvato,0) AS PrezzoPromoIvato, COALESCE(v.InOfferta,0) AS InOfferta, v.OfferteDataFine, ")
        ...
        sql.Append(" WHERE d.TipoDocumentiId = 4 AND YEAR(d.DataDocumento) = YEAR(CURDATE())")
    ...
    Private Function GetRecentlyViewedProducts(ByVal limit As Integer, ByVal fallback As DataTable) As DataTable
        ...
        sql.Append(" GROUP BY dr.ArticoliId")
        sql.Append(") s ON s.ArticoliId = v.id ")
        sql.Append("WHERE COALESCE(v.NListino,1)=@listino AND v.id IN (").Append(idsCsv).Append(") ")
        sql.Append("GROUP BY v.id ORDER BY ").Append(orderSql.ToString())
    ...
    Private Function TakeDistinctRows(ByVal count As Integer, ByVal used As HashSet(Of Integer), ParamArray ByVal sources() As DataTable) As DataTable
        ...
        If result.Rows.Count < count Then
            AddDistinctRows(result, count, Nothing, seen, True, GetFallbackProducts(count))
        End If
    ...
    Private Function EnsureMinimumRows(ByVal source As DataTable, ByVal minimum As Integer) As DataTable
        Dim work As DataTable = If(source IsNot Nothing, source.Copy(), SampleProducts(0).Clone())
        If work.Rows.Count >= minimum Then
            Return work
        End If

        Dim fallback As DataTable = GetFallbackProducts(minimum)
        Dim i As Integer = 0
    ...
    Private Function GetFallbackProducts(ByVal limit As Integer) As DataTable
        Dim sql As String = "SELECT a.id, a.Codice, a.Ean, a.Descrizione1, a.Descrizione2, IFNULL(a.DescrizioneLunga,'') AS DescrizioneLunga, '' AS MarcheDescrizione, a.Img1, a.Img2, a.Img3, a.Img4, 0 AS Vetrina, a.DataCreazione, COALESCE(a.Visite,0) AS Visite, 0 AS Stato, COALESCE(g.Disponibilita,0) AS Giacenza, COALESCE(g.Disponibilita,0) AS Disponibilita, 0 AS Impegnata, COALESCE(al.PrezzoIvato,0) AS PrezzoIvato, 0 AS PrezzoPromoIvato, 0 AS InOfferta, NULL AS OfferteDataFine, 0 AS QtaVenduta FROM articoli a LEFT JOIN articoli_listini al ON al.ArticoliId = a.id AND al.NListino = 1 LEFT JOIN (SELECT ArticoliId, SUM(Disponibilita) AS Disponibilita FROM articoli_giacenze GROUP BY ArticoliId) g ON g.ArticoliId = a.id WHERE COALESCE(a.Abilitato,1)=1 ORDER BY COALESCE(a.Visite,0) DESC, a.id DESC LIMIT " & Math.Max(1, limit).ToString(CultureInfo.InvariantCulture)
    ...
    Private Function SampleProducts(ByVal limit As Integer) As DataTable
        ...
        Dim sample = {
            New Object() {1, "NB-GAME-01", "800000000001", "Notebook Gaming", "Prestazioni elevate", "Notebook gaming per prestazioni elevate", "KeepStore", "laptop.webp", "", "", "", 1, Date.Today.AddDays(-5), 120, 0, 12D, 12D, 0D, 1499.9D, 1299.9D, 1, Date.Today.AddDays(5).ToString("yyyy-MM-dd"), 44D},
            New Object() {2, "TV-4K-02", "800000000002", "Smart TV 4K", "Cinema experience", "Smart TV 4K per home cinema", "KeepStore", "tivi.webp", "", "", "", 1, Date.Today.AddDays(-10), 98, 0, 25D, 25D, 0D, 799.9D, 699.9D, 1, Date.Today.AddDays(3).ToString("yyyy-MM-dd"), 31D},
            New Object() {3, "CAM-03", "800000000003", "Mirrorless Camera", "Creator edition", "Mirrorless per creator e streaming", "KeepStore", "camera-1.webp", "", "", "", 0, Date.Today.AddDays(-2), 88, 0, 8D, 8D, 0D, 1099.9D, 0D, 0, Date.Today.AddDays(7).ToString("yyyy-MM-dd"), 18D},
            New Object() {4, "MSE-04", "800000000004", "Mouse Wireless", "Everyday office", "Mouse wireless per ufficio e mobilita", "KeepStore", "camera-2.webp", "", "", "", 0, Date.Today.AddDays(-1), 40, 0, 64D, 64D, 0D, 39.9D, 29.9D, 1, Date.Today.AddDays(2).ToString("yyyy-MM-dd"), 56D},
            New Object() {5, "HDP-05", "800000000005", "Gaming Headset", "Audio surround", "Cuffie gaming con audio surround", "KeepStore", "camera-3.webp", "", "", "", 1, Date.Today.AddDays(-8), 75, 34, 40D, 40D, 0D, 129.9D, 99.9D, 1, Date.Today.AddDays(4).ToString("yyyy-MM-dd"), 23D}
        }
    ...
    Protected Function CartAddUrl(ByVal id As Object) As String
        Return "cart_add.aspx?id=" & HttpUtility.UrlEncode(Convert.ToString(id))
    End Function
    ...
    Protected Function CurrentPrice(ByVal priceIvato As Object, ByVal promoIvato As Object, ByVal inOfferta As Object) As Decimal
        ...
        Dim promo As Decimal = ToDecimal(promoIvato)
        Dim offerta As Integer = SafeInt(inOfferta)
        If offerta = 1 AndAlso promo > 0D Then
            Return promo
        End If
        Return listino
    End Function
    ...
    Protected Function SavingsAmount(ByVal priceIvato As Object, ByVal promoIvato As Object, ByVal inOfferta As Object) As Decimal
        Dim listino As Decimal = ToDecimal(priceIvato)
        Dim current As Decimal = CurrentPrice(priceIvato, promoIvato, inOfferta)
    ...
    Protected Function AvailabilityPercent(ByVal giacenza As Object, ByVal sold As Object) As Integer
        ...
        Return Convert.ToInt32(Math.Round((soldCount / total) * 100D, MidpointRounding.AwayFromZero))
    End Function
    ...
    Private Function RenderActionButtons(ByVal row As DataRow, ByVal compact As Boolean) As String
        ...
        Dim priceText As String = FormatMoney(CurrentPrice(row("PrezzoIvato"), row("PrezzoPromoIvato"), row("InOfferta")))
        Dim cssClass As String = If(compact, "ks-card-actions is-compact", "ks-card-actions")

        Dim sb As New StringBuilder()
        sb.Append("<div class='").Append(cssClass).Append("'>")
        sb.Append("<a href='").Append(CartAddUrl(row("id"))).Append("' class='ks-action-btn is-cart'>Aggiungi al carrello</a>")
        sb.Append("<a href='").Append(WishlistAddUrl(row("id"))).Append("' class='ks-action-btn is-wishlist'>Wishlist</a>")
        sb.Append("<a href='compare.aspx?add=").Append(EncodeAttr(idText)).Append("' class='ks-action-btn is-compare js-ks-compare' data-ks-id='").Append(EncodeAttr(idText)).Append("' data-ks-title='").Append(EncodeAttr(title)).Append("' data-ks-url='").Append(EncodeAttr(url)).Append("' data-ks-img='").Append(EncodeAttr(img)).Append("' data-ks-price='").Append(EncodeAttr(priceText)).Append("'>Confronta</a>")
        sb.Append("<a href='").Append(url).Append("' class='ks-action-btn is-view'>Vedi prodotto</a>")
        sb.Append("</div>")
        Return sb.ToString()
    End Function

    Private Function RenderSaleBadge(ByVal row As DataRow) As String
        If row Is Nothing OrElse Not ShowDiscount(row("PrezzoIvato"), row("PrezzoPromoIvato"), row("InOfferta")) Then
            Return String.Empty
        End If

        Return "<div class='box-sale-wrap top-0 start-0 z-5'><p class='small-text'>Offerta</p><p class='title-sidebar-2'>" &
               DiscountPercent(row("PrezzoIvato"), row("PrezzoPromoIvato"), row("InOfferta")).ToString(ItCulture) &
               "%</p></div>"
    End Function
    ...
    Private Function RenderPriceBlock(ByVal row As DataRow, ByVal emphasize As Boolean) As String
        Dim sb As New StringBuilder()
        Dim priceClass As String = If(emphasize, "new-price h4 fw-normal text-primary mb-0", "new-price h6 fw-normal text-primary mb-0")

        sb.Append("<p class='price-wrap fw-medium'>")
        sb.Append("<span class='").Append(priceClass).Append("'>").Append(FormatMoney(CurrentPrice(row("PrezzoIvato"), row("PrezzoPromoIvato"), row("InOfferta")))).Append("</span>")
    ...
    Protected Function RenderDealCard(ByVal dataItem As Object) As String
        ...
        sb.Append("</div>")
        If ShowDiscount(row("PrezzoIvato"), row("PrezzoPromoIvato"), row("InOfferta")) Then
            sb.Append("<p class='box-sale-tag'>Risparmi ").Append(FormatMoney(SavingsAmount(row("PrezzoIvato"), row("PrezzoPromoIvato"), row("InOfferta")))).Append("</p>")
        End If
        ...
        sb.Append("<div class='progress-bar bg-danger' style='width:").Append(AvailabilityPercent(row("Giacenza"), row("QtaVenduta")).ToString(ItCulture)).Append("%'></div>")
        sb.Append("</div>")
        sb.Append("<div class='box-quantity d-flex justify-content-between'>")
        sb.Append("<p class='text-avaiable caption'>Venduti: <span class='fw-bold'>").Append(FormatQuantity(row("QtaVenduta"))).Append("</span></p>")
