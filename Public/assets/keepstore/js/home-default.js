(function () {
  ...
  function cloneOnsusGridCardClean(card) {
    var clone = null;
    if (!card) return null;
    if (card.classList && card.classList.contains('ks-onsus-grid-card')) {
      clone = card.cloneNode(true);
      clone.className = 'ks-onsus-grid-card';
    ...
    return onsusCloneGridCardFromNode(card);
  }

  function onsusCardDedupeKey(card) {
    if (!card) return '';
    var href = (card.getAttribute('href') || '').toLowerCase();
    var id = productIdFromUrl(href || '');
    if (id) return 'id:' + id;
    var title = textOf(q('.ks-onsus-title', card) || card).toLowerCase();
    return 'title:' + productBridgeFamilyKey({ title: title, meta: textOf(q('.ks-onsus-meta', card) || '') });
  }

  function enforceOnsusTemplateProductDeck() {
    if (!isHome()) return;
    var bridge = q('#KsHomeOnsusBridge');
    if (!bridge) return;
    var container = q('.container', bridge) || bridge;
    var existingDeck = q('.ks-onsus-main-grid-deck', bridge);
    if (existingDeck && existingDeck.getAttribute('data-ks-template-deck') === '1') { removeHardHide(existingDeck); return; }
    var sources = [];
    qa('.ks-onsus-center .ks-onsus-grid-card,.ks-onsus-side-card,.ks-onsus-product-strip .ks-onsus-grid-card,.ks-onsus-extra-grid .ks-onsus-grid-card', bridge).forEach(function (card) {
      if (!card || card.hidden || card.classList.contains('ks-onsus-duplicate-removed') || card.classList.contains('ks-onsus-overflow-removed') || card.classList.contains('ks-onsus-best-duplicate')) return;
      if (card.closest('.ks-onsus-main-grid-deck')) return;
      if (!q('img', card) || !q('.ks-onsus-title', card)) return;
      sources.push(card);
    });
    ...
    sources.forEach(function (card) {
      ...
      var clone = cloneOnsusGridCardClean(card);
      if (!clone) return;
      var key = onsusCardDedupeKey(clone);
      if (!key || seen[key]) return;
      seen[key] = 1;
      nodes.push(clone);
    });
    ...
    deck.className = 'ks-onsus-main-grid-deck';
    deck.setAttribute('data-ks-template-deck', '1');
    nodes.forEach(function (node) { deck.appendChild(node); });
    var title = q('.ks-home-section-title', bridge);
    if (title && title.nextSibling) container.insertBefore(deck, title.nextSibling); else container.appendChild(deck);
    qa('.ks-onsus-feature-grid,.ks-onsus-product-strip,.ks-onsus-extra-grid', bridge).forEach(function (oldGroup) {
      if (!oldGroup || oldGroup === deck || oldGroup.closest('.ks-onsus-main-grid-deck')) return;
      oldGroup.setAttribute('hidden', 'hidden');
      oldGroup.style.setProperty('display', 'none', 'important');
    });
    bridge.classList.add('ks-onsus-template-grid-mode');
    bridge.setAttribute('data-ks-onsus-template-grid', '1');
    if (document.body) document.body.classList.add('ks-home-step99-onsus-template-grid');
    bindGeneratedImageFallbacks(deck);
  }

  function normalizeBestSellerAsOnsusCarouselRow() {
    if (!isHome()) return;
    var sections = qa('main section').filter(function (sec) { return /best seller/i.test(textOf(sec)) && countArticleLinks(sec) >= 3; });
    var best = sections[0];
    if (!best) return;
    var cards = [];
    qa('a[href*="articolo.aspx?id="]', best).forEach(function (link) {
      var card = link.closest('.swiper-slide,.card-product,.ks-grid-card,li,.col,.product-item') || link;
      if (!card || cards.indexOf(card) >= 0) return;
      if (!q('img', card)) return;
      cards.push(card);
    });
    if (cards.length <= 5) return;
    cards.forEach(function (card, index) {
      if (index >= 5) {
        card.classList.add('ks-best-seller-overflow');
        card.setAttribute('hidden', 'hidden');
        card.style.setProperty('display', 'none', 'important');
      }
    });
    best.setAttribute('data-ks-onsus-best-row', '1');
    if (document.body) document.body.classList.add('ks-home-step99-best-row');
  }



  function hideOnsusTemplateSourceGroups(bridge) {
    if (!bridge) return;
    qa('.ks-onsus-feature-grid,.ks-onsus-product-strip,.ks-onsus-extra-grid,.ks-onsus-side-col,.ks-onsus-center', bridge).forEach(function (node) {
      if (!node || node.classList.contains('ks-onsus-main-grid-deck') || node.closest('.ks-onsus-main-grid-deck')) return;
      node.classList.add('ks-onsus-source-hidden');
  ...
  function enforceOnsusTemplateDeckOrderV2() {
    if (!isHome()) return;
    var bridge = q('#KsHomeOnsusBridge');
    if (!bridge) return;
    var deck = q('.ks-onsus-main-grid-deck[data-ks-template-deck="1"]', bridge) || q('.ks-onsus-main-grid-deck', bridge);
    if (!deck) return;

    removeHardHide(deck);
    deck.removeAttribute('hidden');
    deck.style.removeProperty('display');
    deck.style.removeProperty('visibility');
    deck.style.removeProperty('height');
    deck.style.removeProperty('min-height');
    deck.style.removeProperty('margin');
    deck.style.removeProperty('padding');

    bridge.classList.add('ks-onsus-template-grid-mode');
    bridge.classList.add('ks-onsus-template-grid-mode-v2');
    bridge.setAttribute('data-ks-onsus-template-grid', '1');
    bridge.setAttribute('data-ks-onsus-phase2', '1');
    if (document.body) document.body.classList.add('ks-home-step100-onsus-phase2');

    var container = q('.container', bridge) || bridge;
    var header = q('.ks-home-section-title', bridge);
    if (header && header.parentNode === container && header.nextElementSibling !== deck) {
      container.insertBefore(deck, header.nextSibling);
    } else if (deck.parentNode !== container) {
      container.appendChild(deck);
    }

    hideOnsusTemplateSourceGroups(bridge);

    var cards = [];
    qa('.ks-onsus-grid-card', deck).forEach(function (card) {
      if (!card || card.hidden || card.classList.contains('ks-onsus-overflow-removed')) return;
      if (!q('img', card) || !q('.ks-onsus-title', card)) return;
      cards.push(card);
    });
    cards.forEach(function (card, index) {
  ...
  function productCardRootFromLink(link) {
    if (!link) return null;
    var selectors = ['.swiper-slide', '.card-product', '.ks-grid-card', '.product-item', '.product-card', '.item', 'li', '.col', '[class*="col-"]'];
    for (var i = 0; i < selectors.length; i += 1) {
      var root = link.closest(selectors[i]);
      if (root && q('img', root) && root !== document.body) return root;
    }
    return link;
  }

  function findBestSellerBlockV2() {
    ...
    for (var i = 0; i < headings.length; i += 1) {
      ...
      var guard = 0;
      while (root && root !== document.body && guard < 8) {
        if (countArticleLinks(root) >= 3) return root;
  ...
  function normalizeBestSellerAsOnsusCarouselRowV2() {
    if (!isHome()) return;
    var best = findBestSellerBlockV2();
    if (!best) return;
    var seen = {}, cards = [];
    qa('a[href*="articolo.aspx?id="],a[href*="articolo.aspx?Id="],a[href*="articolo.aspx?ID="]', best).forEach(function (link) {
      var href = link.getAttribute('href') || '';
      var id = productIdFromUrl(href) || href.toLowerCase();
      if (!id || seen[id]) return;
      var card = productCardRootFromLink(link);
      if (!card || !q('img', card)) return;
      seen[id] = 1;
      cards.push(card);
    });
    ...
    cards.forEach(function (card, index) {
      if (index < 5) {
        removeHardHide(card);
        card.classList.remove('ks-best-seller-overflow');
        card.classList.add('ks-best-seller-visible');
    ...
    best.setAttribute('data-ks-onsus-best-row', '1');
    best.setAttribute('data-ks-onsus-best-row-v2', '1');
    if (document.body) document.body.classList.add('ks-home-step100-best-row-v2');
  }


  function bestSellerTitleNodeV3(best) {
    if (!best) return null;
    var nodes = qa('h1,h2,h3,h4,h5,h6,.flat-title,.section-title,.heading-section,.title-sidebar', best);
    for (var i = 0; i < nodes.length; i += 1) {
      if (/best\s*seller/i.test(textOf(nodes[i]))) return nodes[i];
    }
    return null;
  }

  function bestSellerCardRootStrictV3(link) {
    if (!link) return null;
    var selectors = ['.swiper-slide', '.card-product', '.ks-grid-card', '.product-item', '.product-card', '.item-product', 'li', '[class*="col-"]'];
    for (var i = 0; i < selectors.length; i += 1) {
      var root = link.closest(selectors[i]);
      if (root && root !== document.body && q('img', root)) return root;
    }
    return link;
  }

  function rebuildBestSellerAsStableGridV3() {
    if (!isHome()) return;
    var best = findBestSellerBlockV2 ? findBestSellerBlockV2() : null;
    if (!best) return;

    var titleNode = bestSellerTitleNodeV3(best);
    var seen = {}, data = [], roots = [];
    qa('a[href*="articolo.aspx?id="],a[href*="articolo.aspx?Id="],a[href*="articolo.aspx?ID="]', best).forEach(function (link) {
      if (!link || link.closest('.ks-best-seller-grid-v3')) return;
      var href = link.getAttribute('href') || '';
      var id = productIdFromUrl(href) || href.toLowerCase();
      if (!id || seen[id]) return;
      var root = bestSellerCardRootStrictV3(link);
    ...
    var deck = q('.ks-best-seller-grid-v3', best);
    if (!deck) {
      deck = document.createElement('div');
      deck.className = 'ks-best-seller-grid-v3';
      deck.setAttribute('data-ks-best-seller-grid-v3', '1');
    }
    deck.innerHTML = data.slice(0, 5).map(function (item) {
      return bridgeGridCard(item).replace('class="ks-onsus-grid-card"', 'class="ks-onsus-grid-card ks-best-seller-grid-card"');
    }).join('');

    var anchor = titleNode;
    if (anchor && anchor.classList && (anchor.classList.contains('flat-title') || anchor.classList.contains('section-title') || anchor.classList.contains('heading-section'))) {
      anchor.parentNode.insertBefore(deck, anchor.nextSibling);
    } else if (anchor && anchor.parentNode) {
      var titleWrap = anchor.closest('.flat-title,.section-title,.heading-section') || anchor;
      titleWrap.parentNode.insertBefore(deck, titleWrap.nextSibling);
    } else {
      best.insertBefore(deck, best.firstChild);
    }

    roots.forEach(function (root) {
      if (!root || root.closest('.ks-best-seller-grid-v3')) return;
      root.classList.add('ks-best-seller-original-hidden-v3');
    ...
    qa('.tf-sw-products,.swiper,.swiper-wrapper,.product-list-wrap,.tf-grid-layout,.row', best).forEach(function (wrap) {
      if (!wrap || wrap.closest('.ks-best-seller-grid-v3') || wrap.contains(deck)) return;
      var visibleLinks = qa('a[href*="articolo.aspx?id="]', wrap).filter(function (link) {
        var r = bestSellerCardRootStrictV3(link);
        return r && !r.classList.contains('ks-best-seller-original-hidden-v3') && !r.hidden;
      });
      if (visibleLinks.length === 0) {
        wrap.classList.add('ks-best-seller-source-wrap-hidden-v3');
    ...
    best.setAttribute('data-ks-best-seller-stable-grid-v3', '1');
    best.classList.add('ks-best-seller-stable-grid-v3-section');
    if (document.body) document.body.classList.add('ks-home-step101-best-grid-v3');
    bindGeneratedImageFallbacks(deck);
  }
  ...
  function buildHomeCompositionLayer() {
    ...
    runSafe('enforceOnsusTemplateProductDeck', enforceOnsusTemplateProductDeck);
    runSafe('enforceOnsusTemplateDeckOrderV2', enforceOnsusTemplateDeckOrderV2);
