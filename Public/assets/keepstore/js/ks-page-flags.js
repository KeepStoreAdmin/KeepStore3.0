(function () {
  ...
  function containsToken(raw) {
    ...
    return ['welcome', 'franchis', 'onsus', 'themeforest', 'mediacom', 'cellulare', 'tablet', 'monitor'].some(function (token) {
  ...
  function primaryLaneRect() {
    ...
    for (var i = 0; i < candidates.length; i += 1) {
      ...
      if (rect && rect.width > 700 && rect.top < window.innerHeight * 0.75) return rect;
    }
  ...
  function hideHeaderClones() {
    ...
    Array.prototype.slice.call(document.querySelectorAll('div,section,header')).forEach(function (node) {
      ...
      if (rect.width < 420 || rect.height < 28) return;
      ...
      var looksLikeHeader = (
        ...
        text.indexOf('cerca prodotti codici o ean') !== -1 ||
  ...
  function hideTokenArtifacts() {
    ...
    Array.prototype.slice.call(document.querySelectorAll('div,span,p,section,aside,img,a')).forEach(function (node) {
      ...
      if (!outsideLane && rect.width > 220) return;
      var raw = [
        node.id || '',
        node.className || '',
  ...
  function hideRepeatedDeviceRails() {
    ...
    Array.prototype.slice.call(document.querySelectorAll('img')).forEach(function (img) {
      ...
      var rect = rectOf(img);
      if (!rect || rect.width < 40 || rect.height < 60 || rect.width > 180 || rect.height > 260) return;
      var outsideLane = rect.right < lane.left - 12 || rect.left > lane.right + 12;
    ...
    Object.keys(groups).forEach(function (src) {
      ...
      groups[src].forEach(function (img) {
        var root = img.closest('aside, section, div') || img;
        hideNode(root, 'data-ks-edge-creative');
      });
    });
  }
  ...
  function hideOrphanFragments() {
    ...
    Array.prototype.slice.call(document.querySelectorAll('p,span,small,div')).forEach(function (node) {
      if (!node || isProtected(node)) return;
      var rect = rectOf(node);
      if (!rect || rect.width > 260 || rect.height > 120) return;
      var outsideLane = rect.right < lane.left - 12 || rect.left > lane.right + 12;
      if (!outsideLane) return;
  ...
  onReady(function () {
    ...
    runHomeCleanup();
    if (isHomePage()) {
      [250, 1200, 3200, 7000].forEach(function (delay) {
