...
Partial Public Class _Default
    ...
    Private Function GetFeaturedPool(ByVal limit As Integer) As DataTable
        Return QueryProducts("COALESCE(v.Vetrina,0)=1 AND " & StockWhereClause() & ">=1", "COALESCE(v.Vetrina,0) DESC, COALESCE(v.Visite,0) DESC, COALESCE(v.DataCreazione,CURDATE()) DESC, v.id DESC", limit)
    End Function

    Private Function GetMostViewedPool(ByVal limit As Integer) As DataTable
        Return QueryProducts(StockWhereClause() & ">=1", "COALESCE(v.Visite,0) DESC, COALESCE(v.DataCreazione,CURDATE()) DESC, v.id DESC", limit)
    End Function

    Private Function GetNewArrivalsPool(ByVal limit As Integer) As DataTable
        Return QueryProducts(StockWhereClause() & ">=1", "COALESCE(v.DataCreazione,CURDATE()) DESC, COALESCE(v.Visite,0) DESC, v.id DESC", limit)
    End Function

    Private Function GetBestSellerPool(ByVal limit As Integer) As DataTable
        Return QueryProducts(StockWhereClause() & ">=1", "COALESCE(s.QtaVenduta,0) DESC, COALESCE(v.Visite,0) DESC, COALESCE(v.DataCreazione,CURDATE()) DESC, v.id DESC", limit)
    End Function
    ...
    Private Function GetRecentlyViewedProducts(ByVal limit As Integer, ByVal fallback As DataTable) As DataTable
        ...
        Dim dt As DataTable = TryLoadProducts("v.id IN (" & idsCsv & ")", orderSql.ToString(), limit)
        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
    ...
    Private Function OfferWhereClause() As String
        Return "COALESCE(v.InOfferta,0)=1 AND (COALESCE(v.PrezzoPromo,0)>0 OR COALESCE(v.PrezzoPromoIvato,0)>0) AND " & StockWhereClause() & ">=1"
    End Function
    ...
    Private Function RenderSaleBadge(ByVal row As DataRow) As String
        ...
        Return "<div class='box-sale-wrap top-0 start-0 z-5'><p class='small-text'>Promo</p><p class='title-sidebar-2'>" &
    ...
    Private Function RenderRowCardFromRow(ByVal row As DataRow) As String
        ...
        End If
        sb.Append("<h6><a href='").Append(ProductUrl(row("id"))).Append("' class='name-product text-secondary link'>").Append(ProductTitle(row("Descrizione1"), row("Descrizione2"), row("id"))).Append("</a></h6></div>")
    ...
    Protected Function RenderBigCard(ByVal dataItem As Object) As String
        ...
        If Not String.IsNullOrWhiteSpace(Convert.ToString(row("MarcheDescrizione"))) Then
            sb.Append("<p class='caption text-main-2 font-2'>").Append(HttpUtility.HtmlEncode(Convert.ToString(row("MarcheDescrizione")))).Append("</p>")
        End If
        sb.Append("<h5><a href='").Append(ProductUrl(row("id"))).Append("' class='name-product text-secondary link'>").Append(ProductTitle(row("Descrizione1"), row("Descrizione2"), row("id"))).Append("</a></h5>")
        sb.Append(RenderPriceBlock(row, True))
        sb.Append("</div></div>")
        Return sb.ToString()
    End Function
