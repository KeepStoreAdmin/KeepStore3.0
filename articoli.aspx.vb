...
End Sub
    ...
    Public Sub CaricaArticoli()
        ...
        If userCerca <> "" Then
            ...
            scoreBuilder.Append("WHEN LOWER(CONCAT(' ',COALESCE(MarcheDescrizione,''),' ',COALESCE(Descrizione1,''),' ',COALESCE(Descrizione2,''))) LIKE ?qLike THEN 4300000 ")
            scoreBuilder.Append("WHEN LOWER(CONCAT(' ',COALESCE(SettoriDescrizione,''),' ',COALESCE(CategorieDescrizione,''),' ',COALESCE(TipologieDescrizione,''),' ',COALESCE(Descrizione1,''))) LIKE ?qLike THEN 3900000 ")
            ...
            For i As Integer = 0 To searchTokens.Count - 1
                ...
                scoreBuilder.Append(" + (CASE ")
                scoreBuilder.Append("WHEN LOWER(Codice)=?").Append(tokenName).Append(" THEN 9000 ")
                scoreBuilder.Append("WHEN LOWER(Ean)=?").Append(tokenName).Append(" THEN 8800 ")
                scoreBuilder.Append("WHEN LOWER(Descrizione1)=?").Append(tokenName).Append(" THEN 8600 ")
                scoreBuilder.Append("WHEN LOWER(Codice) LIKE ?").Append(tokenPrefixName).Append(" OR LOWER(CONCAT(' ',COALESCE(Codice,''))) LIKE CONCAT('% ',?").Append(tokenName).Append(",'%') THEN 6200 ")
                scoreBuilder.Append("WHEN LOWER(Ean) LIKE ?").Append(tokenPrefixName).Append(" OR LOWER(CONCAT(' ',COALESCE(Ean,''))) LIKE CONCAT('% ',?").Append(tokenName).Append(",'%') THEN 6000 ")
                scoreBuilder.Append("WHEN LOWER(Descrizione1) LIKE ?").Append(tokenPrefixName).Append(" OR LOWER(CONCAT(' ',COALESCE(Descrizione1,''))) LIKE CONCAT('% ',?").Append(tokenName).Append(",'%') THEN 5800 ")
                scoreBuilder.Append("WHEN LOWER(CONCAT(IFNULL(MarcheDescrizione,''),' ',IFNULL(Descrizione1,''))) LIKE ?").Append(tokenLikeName).Append(" THEN 5200 ")
                scoreBuilder.Append("WHEN LOWER(CONCAT(IFNULL(SettoriDescrizione,''),' ',IFNULL(CategorieDescrizione,''),' ',IFNULL(TipologieDescrizione,''),' ',IFNULL(Descrizione1,''))) LIKE ?").Append(tokenLikeName).Append(" THEN 5000 ")
                scoreBuilder.Append("WHEN LOWER(CONCAT(IFNULL(MarcheDescrizione,''),' ',IFNULL(Descrizione1,''),' ',IFNULL(Descrizione2,''))) LIKE ?").Append(tokenLikeName).Append(" THEN 3600 ")
                scoreBuilder.Append("WHEN LOWER(Descrizione1) LIKE ?").Append(tokenLikeName).Append(" THEN 2600 ")
                scoreBuilder.Append("WHEN LOWER(DescrizioneLunga) LIKE ?").Append(tokenLikeName).Append(" THEN 1800 ")
                scoreBuilder.Append("WHEN LOWER(MarcheDescrizione) LIKE ?").Append(tokenLikeName).Append(" THEN 1600 ")
                scoreBuilder.Append("WHEN LOWER(CONCAT(IFNULL(SettoriDescrizione,''),' ',IFNULL(CategorieDescrizione,''),' ',IFNULL(TipologieDescrizione,''),' ',IFNULL(GruppiDescrizione,''),' ',IFNULL(SottogruppiDescrizione,''))) LIKE ?").Append(tokenLikeName).Append(" THEN 1500 ")
                scoreBuilder.Append("WHEN LOWER(DescrizioneHTML) LIKE ?").Append(tokenLikeName).Append(" THEN 1200 ")
                scoreBuilder.Append("WHEN LOWER(Descrizione2) LIKE ?").Append(tokenLikeName).Append(" THEN 1000 ")
                scoreBuilder.Append("ELSE 0 END)")
