...
End Sub
    ...
    Private Function GestisciAggiuntaArticoli() As String
        ...
        If SelezioneMultipla.Count > 0 Then
            ...
            For i = 0 To SelezioneMultipla.Count - 1
                ...
                If quantitaRiga <= 0 Then quantitaRiga = 1
                ...
                quantitaRiga += SumCartRowQuantities(equivalentCartRows)
    ...
    Private Function LoadEquivalentCartRows(ByVal loginId As Integer,
        ...
        Return ExecuteQueryGetDataReader("id, qnt", "carrello", wherePart & " ORDER BY id", params)
    End Function
    ...
    Private Function SumCartRowQuantities(ByVal rows As List(Of Dictionary(Of String, Object))) As Double
        ...
        For Each row As Dictionary(Of String, Object) In rows
            Dim qnt As Double = 0
            TryParseDouble(RowObject(row, "Qnt"), qnt)
            total += qnt
        Next
    ...
    Private Function CollectCartRowIds(ByVal rows As List(Of Dictionary(Of String, Object))) As List(Of Integer)
        ...
        For Each row As Dictionary(Of String, Object) In rows
            Dim rawId As Object = RowObject(row, "id")
            Dim rowId As Integer = 0
            If Integer.TryParse(Convert.ToString(rawId), rowId) AndAlso rowId > 0 Then
                ids.Add(rowId)
            End If
        Next
    ...
    Private Sub DeleteDeferredCartRows(ByVal rowIds As List(Of Integer))
        ...
        Dim deleted As New Dictionary(Of Integer, Boolean)()
        For Each rowId As Integer In rowIds
            If rowId <= 0 OrElse deleted.ContainsKey(rowId) Then Continue For
            DeleteDeferredCartRow(rowId)
            deleted(rowId) = True
        Next
    End Sub

    Private Function RowObject(ByVal row As Dictionary(Of String, Object), ByVal fieldName As String) As Object
